...
배경(260705): dart_finstat_y/q는 각 연도 공시통화 그대로 저장(통화컬럼 없음). 두산밥캣(241560)처럼 연도별로
통화가 바뀌는 회사가 있어, dart_fundamentals.currency(최신 통화 1개)를 전 연도에 곱하는 하위 FX가 옛 연도를
망침(4조×1185=4,826조). → 저장 단계에서 원화 통일하면 하위 FX 불필요.

방법:
  · 대상 = dart_fundamentals.currency ∉ (KRW,nodata,?) 인 22사(USD/CNY/JPY).
...
DART 0콜(krx_weekly·ecos_fx_rate 캐시). 기본 dry-run. 실제 반영은 --apply.
...
async def main(apply: bool) -> None:
    ...
    # orig_currency 보존열
    con.execute("ALTER TABLE dart_fundamentals ADD COLUMN IF NOT EXISTS orig_currency text")
    ...
    firms = con.execute(
        "SELECT ticker, upper(COALESCE(orig_currency, currency)) FROM dart_fundamentals "
    ...
    for isu, ccy in firms:
        ...
        for price_dd, mktcap in con.execute(
                "SELECT price_dd, mktcap FROM krx_weekly WHERE ticker=%s ORDER BY price_dd", (isu,)):
            capfy[int(price_dd[:4])] = float(mktcap or 0)   # 연내 최신(=연말) 덮어씀
        # 연도별 통화 결정 (eq 기준)
        hist = {int(fy): (ni, eq, nir, eqr) for fy, ni, eq, nir, eqr in con.execute(
            "SELECT fy, ni, eq, ni_restated, eq_restated FROM dart_finstat_y WHERE ticker=%s", (isu,))}
        ...
        if apply:
            for fy in conv:
                ...
                con.execute("UPDATE dart_finstat_y SET ni=%s, eq=%s, ni_restated=%s, eq_restated=%s "
                            "WHERE ticker=%s AND fy=%s", (*vals, isu, fy))
                tot_hist += 1
            # 분기: 그 fy의 통화결정을 그대로 적용
            for fy, q, ni, eq in con.execute(
                    "SELECT fy, quarter, ni_cum, eq FROM dart_finstat_q WHERE ticker=%s", (isu,)):
                if decisions.get(int(fy), "").startswith("foreign"):
                    f = await fx(ccy, int(fy))
                    if not f: continue
                    con.execute("UPDATE dart_finstat_q SET ni_cum=%s, eq=%s WHERE ticker=%s AND fy=%s AND quarter=%s",
                                (ni * f if ni is not None else None, eq * f if eq is not None else None, isu, fy, q))
                    tot_q += 1
            con.execute("UPDATE dart_fundamentals SET orig_currency=COALESCE(orig_currency,%s), currency='KRW' WHERE ticker=%s", (ccy, isu))
        print()
    if apply:
        print(f"✓ APPLY: dart_finstat_y {tot_hist}행 · dart_finstat_q {tot_q}행 환산 · 라벨 KRW화 + orig_currency 보존")
        print("  → 다음: derive_fundamentals 재실행(dart_fundamentals 원화화) + 밴드 재실행")
    con.close()
