"""과거 `tool_call_events.corp_codes` 를 사용자와 떼어 `corp_daily` 로 옮기고 컬럼을 비운다.
...
**연결을 백업하지 않는다.** 백업하면 지운 의미가 없다. 옮겨진 집계(corp_daily)가
...
def main(apply: bool) -> None:
    ...
    rows = con.execute(
        "SELECT ts_ns, corp_codes FROM tool_call_events WHERE corp_codes IS NOT NULL").fetchall()
    if not rows:
    ...
    if not apply:
        ...
        print("  --apply 하면: corp_daily 에 집계 반영 후 tool_call_events.corp_codes 를 NULL 로 비움")
    ...
    con.cursor().executemany(
        ...
        "requests = corp_daily.requests + EXCLUDED.requests",
        [(d, c, n) for (d, c), n in agg.items()])

    # 집계가 들어간 것을 **확인한 뒤에** 비운다. 순서가 반대면 실패 시 값이 사라진다.
    got = con.execute("SELECT coalesce(sum(requests),0) FROM corp_daily").fetchone()[0]
    want = sum(agg.values())
    if got < want:
        con.rollback()
        print(f"❌ 집계 검증 실패 (corp_daily 합계 {got} < 옮긴 값 {want}) — 아무것도 안 지운다")
        con.close()
        return

    cleared = con.execute(
        "UPDATE tool_call_events SET corp_codes = NULL WHERE corp_codes IS NOT NULL").rowcount
    con.commit()
    print(f"\n✓ corp_daily 반영 (합계 {got:,}) · 이벤트 행 {cleared:,}건에서 기업 연결 제거")
    left = con.execute(
        "SELECT count(*) FROM tool_call_events WHERE corp_codes IS NOT NULL").fetchone()[0]
