...
def diagram(kind):
    ...
    if kind=='runtime':
        boxes=[(35,65,'01 · REQUEST','회차·기준일 고정',['회사 · 회의 · 후보 범위','사후 자료와 대상 회차 결과 제외']),
               (405,65,'02 · EVIDENCE','공개 원문 수집',['공고 · 사업보고서 이사회 절','지정 DART/KIND 공시 최대 5건']),
               ...
               (35,415,'07 · RULES','규칙별 상태와 권고',['반대 신호 보존 · 긍정 게이트','출석 확정 입력은 아직 미구현']),
...
manifest={'generated_from':str(SOURCE.relative_to(ROOT)),'source_sha256':hashlib.sha256(raw.encode()).hexdigest(),
          ...
          'assessment_contract':'opm-llm-assessment/2','document_date':'2026-09-09','status':'local_document_export',
          'verification_note':'0.3.0 real pilot results; 0.4.0 rubric delivery verified, no candidate re-evaluation'}
