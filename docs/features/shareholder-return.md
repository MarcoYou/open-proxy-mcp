# 주주환원

> 예시기업 표기는 익명 사례입니다. 호출 예시에서는 실제 회사명·식별자로 바꾸세요. [표기 기준](../../wiki/wiki_schema.md)

**"약속(정책)"과 "실제 집행(사실)"을 나란히 놓고 주주환원이 지켜졌는지 봅니다.** 배당·자기주식·기업가치제고계획(밸류업)을 묶어 확인합니다.

## 무엇을 답하나

- **배당**: 주당배당금(DPS)·배당금총액, 배당성향(배당÷순이익), 시가배당률(배당기준일 주가 기준), 연도별 추이 — 미래 약속이 아닌 확정·실지급 사실입니다.
- **자기주식**: 취득 → 처분 → 소각 사이클을 매칭합니다. **핵심은 매입(취득) 자체** — 현금은 매입 시점에 이미 지출되므로, 그 뒤 소각까지 이어지는지는 회계 정리 절차일 뿐 주주환원의 실질을 바꾸지 않습니다(소각 없이 보유만 해도 환원 효과는 이미 발생). 대신 최근에 **새로 매입**한 건지, 몇 년 전 사둔 보유분을 지금 처리하는 것뿐인지(신규 자금 투입 여부)를 구분해서 봅니다.
- **밸류업**: 기업가치제고계획의 약속을 실제 자사주 소각 집행과 **교차참조**해 "말뿐인 밸류업 vs 진짜 환원"을 가릅니다.
- 이 셋을 한 번에 보는 전용 기능 `shareholder_commitment`은 약속 대비 실제 이행을 추적하고, 자사주 소각의 **매입가 대비 장부가 손익까지 원화로 계산**합니다.
- 출처: DART 배당결정·사업보고서 배당사항·자기주식·밸류업 공시. 상세 → [dividend](../../wiki/tools/dividend_disclosure.md) · [treasury_share](../../wiki/tools/treasury_share.md) · [value_up](../../wiki/tools/value_up.md).

## 이렇게 물어보세요

> "예시기업 FW 배당이랑 자사주 소각 이력 봐줘"
>
> "이 회사 밸류업 계획 실제로 지켰어?"

## 함께 보면 좋은 기능

- [지분·지배구조 맵](ownership.md) — 자기주식이 지분 구조에서 갖는 의미
- [주총 의결권 보조](proxy-voting.md) — 배당·자사주 안건의 의결권 판단
