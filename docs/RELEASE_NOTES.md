...
이사회 프로필(SK)이 7.8초에서 3초 안팎으로 줄고, 그 사이 서버가 다른 요청과 헬스체크에 답하지 못하던 최대 4~6초 멈춤이 0.4초 이하가 됐습니다. 원인은 보수한도 안건을 읽기 위해 부르는 주총 소집공고 파싱(22MB, 동기)이었습니다. 파싱을 워커 스레드로 옮기고, 요청 범위가 쓰지 않는 표는 건너뛰고, 후보 공고 분류 때 이미 읽은 회의 정보를 재사용합니다. 출력은 같습니다.
