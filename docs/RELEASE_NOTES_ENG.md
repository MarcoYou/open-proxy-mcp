...
The design was fixed by measuring 11 annual reports (manufacturing, mid cap, insurance, bank holding, securities, construction, biotech, retail, REIT, small-cap KOSDAQ, utility): peak memory per section request under 9 MB, text-only cache (0.2–3.3 MB per report), about 1.5 s per section round trip, one retry on transport errors. Viewer requests keep the existing shared web-request clock and share the document concurrency gate. `business_details`, `financial_notes`, `asset_holdings` and `shareholder_meeting_notice` now write these addresses next to weak or missing items, so the reader can open the exact section without going through the table of contents.
