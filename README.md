...
Claude에서는 `+ → 커넥터 → open-proxy-mcp에서 추가 → OpenProxy 기능 안내`를 선택해 `tools_guide`(`opm://tools_guide`)를 대화에 첨부할 수 있습니다. 같은 메뉴에서 `Open Proxy Guideline`도 첨부할 수 있습니다. 기능 안내는 서버에 등록된 도구 목록에서 자동으로 구성되며, 리소스는 필요한 대화에서 사용자가 직접 선택합니다.

MCP 프롬프트를 지원하는 클라이언트에서는 `company_snapshot`(회사 한 장 요약)에 회사명이나 종목코드를 입력하세요. Claude에서는 위와 같은 메뉴의 `회사 한 장 요약`으로 불러올 수 있습니다. 사업 구조·3년 확정 실적·제공되는 연간 예상치 최대 2년·가격·지분·배당·최근 공시를 연결하고, 더 확인할 질문까지 정리하도록 안내합니다. 표에서 확정(A)과 예상(E)을 구분하며, 시각화가 가능한 클라이언트에는 매출 막대·영업이익 선 차트를 요청합니다.
