...
작업 중간 보고와 마무리 보고 모두 이 양식이다. 읽는 사람은 코드를 안 본 팀장이다.

- **결론부터 한 줄.** 「됐습니다 / 안 됐습니다 / 막혔습니다」 중 하나로 시작하고, 근거는 그 뒤에 붙인다.
- **우리말로 풀어 쓴다.** 함수·표·인자·브랜치 이름 같은 내부 이름은 팀장이 직접 열어 봐야 할 때만, 한 문장에 하나. 나머지는 「무엇이 어떻게 되는가」로 말한다. 영어 약어는 처음 나올 때 풀어 준다.
- **숫자는 표에, 문장에는 넣지 않는다.** 결정을 바꾸는 숫자만 싣는다.
- **못 한 것·안 한 것·확인하지 못한 것을 먼저** 말한다. 잘된 것 뒤에 숨기지 않는다.
- **다음 행동은 둘로 나눈다.** 「제가 이어서 할 것」과 「팀장님이 정할 것」. 정할 것은 선택지와 제 권고 하나를 함께.
- **길이는 화면 한 번.** 설계 배경·시행착오·실측 로그는 `wiki/anecdotes/` 에 두고 보고엔 링크만.
- **되묻기 전에 다 한다.** 질문은 팀장만 답할 수 있는 것만, 맨 끝에 한 번.
