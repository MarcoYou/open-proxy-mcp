...
질문이 오면 `open-proxy-mcp/index.md`를 먼저 읽고, 관련 페이지만 선택적으로 읽을 것.
...
### raw/ 절대 수정 금지
`open-proxy-mcp/raw/`는 외부 원본 (운용사 정책 PDF, 행사내역 xlsx, 외부 reference markdown).
...
open-proxy-mcp/        # LLM 도메인 지식 위키 (Karpathy 아키텍처) — 구 wiki/
...
- DART API → `open-proxy-mcp/archive/entities/DART-OpenAPI.md` (구 entity 페이지, archive 보존)
- fallback → `open-proxy-mcp/architecture/3-tier-fallback.md`
- 데이터 수집 전체 → `open-proxy-mcp/architecture/data-collection.md`
- 공시 유형 → `open-proxy-mcp/rules/disclosures/`
- 도메인 개념 → `open-proxy-mcp/rules/concepts/`

## 문서 포인터
- 미완료 작업 → `TO_DO.md` — **완료된 항목은 즉시 삭제. 완료 섹션 없음. 미완료만 유지.**
- 개발 히스토리 → `DEVLOG.md`
- wiki 작업 로그 → `open-proxy-mcp/log.md`
