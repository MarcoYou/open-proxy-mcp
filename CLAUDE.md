...
2. **정확성 = 큰 표본 × 이중 검증.** ① 기계적(스크립트·전수 diff) **그리고** ② 사람-독자 관점(직접 표본을 눈으로 읽음) — 둘 다 한다. 측정 도구의 가정(production 경로·ground truth·패턴 엄격도)을 먼저 의심한다. 사용자가 시키기 전에 default로. 상세·5패턴·체크리스트: private lessons(`~/Projects/open-proxy-storage/wiki-private/lessons/agenda-parser-validation-260621.md`).
   ...
   - **회계·재무 판단이 들어간 로직(자산분류·지표계산·밸류에이션 등)은 회계사 에이전트(K-IFRS 등 회계기준 정합성) + Data QA 에이전트(측정·회귀 검증)로 검토받은 뒤 반영한다.** 사람 직관만으로 분류·수식을 설계하면 잘못된 회계개념을 코드에 그대로 굳힐 위험. 필요시 부동산전문가·가치투자자·공시전문가 등 추가 페르소나 병행. 실측 사례: [[260721_1500_decision_asset-holdings-purpose-buckets]] · private lessons(`.../asset-holdings-census-260720.md`).
...
open_proxy_mcp/
  ...
  tools_v2/            # 26 public tool (active)
