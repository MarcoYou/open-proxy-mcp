...
다음 구현은 출석·독립성·정관 조문 세 흐름의 실제 MCP 출력부터 검증한다. 근거의 주체/기간 오류를 차단하고, 결측 시 원문과 다음 조회 경로가 유효한지 측정한다. 그 이후 사용자 정책과 대량 처리, 정책 갱신·실무 모듈을 확장한다. 이 패키지는 초기 통합 설계 자료다. 현재 진행 브랜치는 `combined_guideline_v1`이며 기존 `astra-fable-opm-guideline-v2`의 커밋 이력을 보존한다. 최신 실행 명세는 [가이드라인 명세](../guideline-specification-20260909/index.html)를 따른다. OPM 운영 코드와 배포는 변경하지 않았다. 문서 사이트 게시는 별도다.
