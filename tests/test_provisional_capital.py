...
def test_a_financial_company_balance_sheet_is_recognised() -> None:
    """금융사는 **총계도 유동/비유동 구분도 안 적는다.**

    K-IFRS 1001호 유동성 순서 배열이라 「자 산」·「부 채」·「자 본」 섹션 헤더만 두고 계정을
    나열한다(실측 20260309001843: 48행 재무상태표에 「총계」 문자가 한 번도 안 나온다).
    총계류만 앵커로 두면 그런 표를 통째로 막아 **자본금·지배지분이 원문에 있는데도 못 뽑는다** —
    표 검증 가드를 넣으면서 만든 회귀였다.
    """
    m = extract_metrics(_bs([
        ["자              산", "", ""],
        ["I. 현금및현금성자산", "1,000", "900"],
        ["부               채", "", ""],
        ["자               본", "", ""],
        ["I. 지배주주지분", "700", "650"],
        ["1.자본금", "100", "100"],
    ...
    assert m["fy_current_capital_stock_krw"] == 100_000_000
    assert m["fy_current_controlling_equity_krw"] == 700_000_000


def test_a_profit_row_is_not_read_as_non_controlling_equity() -> None:
    """「비지배지분순손익」은 자본이 아니라 그 해 손익의 귀속분이다(실측 20260310002815).

    자본 계정으로 읽으면 자본총계에서 그만큼 잘못 빼게 되고, 그 오차가 그대로 지배지분
    오산으로 간다.
    """
    m = _m([["자본금", "1,000", "1,000"],
            ["비지배지분순손익", "77", "60"],
            ["비지배지분총포괄순손익", "80", "62"],
            ["자본총계", "5,000", "4,800"]])
    assert m.get("fy_current_nci_krw") is None
