...
260823: KSIC 하이브리드 하나뿐이던 집계에 업종분류(대분류·중분류)를 더했다. 같은 표
(`opm_val_market`)에 `scheme` 으로 나눠 담는데, `sector != '_ALL'` 만으로 거르던 기존
질의가 **업종분류 행까지 집어와** 섹터 표가 중복될 뻔했다(같은 종목이 두 버킷에 잡힌다).

당시엔 우연히 안 섞였다 — 업종분류 백필이 20260731 까지고 최신 snap_dd 는 20260821 이라
...
def test_scheme_whitelist_and_descriptions():
    assert set(V._SECTOR_SCHEMES) == {"ksic", "대분류", "중분류"}
...
def test_class_company_sector_comes_from_snapshot_table_with_fallback():
    """업종분류는 종목 섹터가 opm_val_firm 이 아니라 분류 스냅샷 표에서 온다.
...
def test_footnote_names_the_actual_scheme_and_discloses_backfill():
    """각주가 「KSIC 하이브리드」로 굳어 있으면 업종분류로 조회해도 KSIC 라고 말한다 —
    사용자가 다른 축을 봤다고 믿게 되는 자리다. 소급도 함께 고지한다."""
    src = inspect.getsource(V.build_sector_val_payload)
    assert "_SRC[scheme]" in src, "각주가 scheme 을 안 따른다"
    assert "소급 적용" in src, "업종분류 과거 구간이 소급이라는 고지가 없다"
