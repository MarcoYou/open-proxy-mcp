...
보통주 표기를 다음과 같이 **매우 다양하게** 쓴다 — `보통주` 외에 `의결권 있는 주식`, `의결권있는 주식`,
`의결권이 있는 주식`, 개행이 끼어든 `의결권\n있는 주식`, 전부 붙은 `의결권있는주식`, `의결권 있는 주식\n(보통주)`
같은 형태가 섞이고, `합  계` 처럼 공백이 들어간 합계 행도 있다.

따라서 `if "보통" in stock_kind` 단일 조건으로는 절대 잡을 수 없음.

> 변형 전수 열거표(회사별 매핑 + KOSPI 200 표본 25종 빈도)는 storage
