...
**동기화 기준 (2026-09-10):** 진행 브랜치는 `claude-fable-5.1-opm-guideline-v2`다. v2 구현 기준 `4bd3674e`에 운영·원격 main 기준 `d1f83d10beb36c8760473588df9284db7f3f1502`의 변경 16개를 통합했다. 이 SHA는 GitHub production 성공 배포 기록과 일치하며 운영 `/health`도 정상 응답했다. v2가 운영에 배포됐다는 뜻은 아니다. 아래 비교 브랜치 정리와 실행 프레임워크를 함께 유지하고, 실제 5개사 품질 검증은 여전히 04의 다음 작업으로 남긴다.

## 실행 프레임워크 및 검증 프로필

기존 MCP의 **원문 발견 → 호출 LLM 판독 → 평가 제출 → 규칙·제약 적용**을 사용한다. 실행 원칙과 기본 프로필은 이 절에 두며, 도구 인자의 정본은 [[proxy_advise_before_meeting]]·[[governance_screen]], 설정 타입은 `open_proxy_mcp/services/guideline_workflow.py`와 내려받을 수 있는 `workflow.schema.json`이다. 별도 LLM 프레임워크나 두 번째 판정 엔진은 설치하지 않는다.

### 실행 기준과 기본 설정

| 구분 | 다음 검증에 적용할 설정 |
|---|---|
| 회사 범위 | 가비아·골프존홀딩스·태광산업·영풍·고려아연. 최대 30개사 계약을 유지하되 이번 검증은 5개사로 시작한다. |
| 회사 사건 대조 기준일 | `20260909`. 이전 파일럿의 당시 판단을 대조한다. 이후 새 공시는 별도 기준일의 재평가로 구분한다. |
| 주총·후보 대조 기준일 | 회차와 정보 기준일을 먼저 고정하고, 해당 소집공고·정기/임시 구분을 확인한다. 회사 사건의 사후 결과를 주총 전 권고에 소급하지 않는다. |
...
프레임워크 준비는 운영 기준 커밋 통합, 회귀·문서 검사, 다시 시작한 파일럿의 실제 MCP 계약 확인으로 닫는다. **5개사의 판단 품질 검증은 그 다음 실행**이다. 확인된 중대한 사실·시점·당사자·정책 적용 오류를 수정하고 해당 사례를 실제 MCP로 재확인하면 그 범위의 검증을 닫는다. 미확인 사항·정책 해석 차이는 설명 가능한 상태로 남기며 전체 흐름을 무기한 기다리게 하지 않는다. 배포에 영향을 주는 중대한 오류가 남은 기능은 그 범위만 출시 대상에서 제외한다.

코드·문서 동기화와 운영 배포는 별개다. 이번 작업은 기능 브랜치와 파일럿의 기준을 맞추는 준비이며, 05의 정책 활성화·복구·수동 인계와 06의 운영 배포 완료로 표시하지 않는다.
...
- `proxy_advise_before_meeting`: 삼성전자 2025년 정기주총·기준일 2025-03-18·standard/selective 프로필에서 안건 15개, LLM 평가 대기 6개·범위 밖 9개를 반환했다. JSON 설정·과업과 Markdown 표시를 확인했으며 평가 제출은 0건이다. 이번 호출은 기존 신제윤 교정 사례의 재판정이나 5개사 독립 품질 검증으로 세지 않는다.
