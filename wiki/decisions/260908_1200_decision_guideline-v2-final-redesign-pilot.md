...
이 절은 0.4.0-pilot 코드의 현재 계약이다. 기사·사건의 정형 필드와 자동 갱신은 뒤의 확장 설계다.
...
| 신임 확정 + no_public_concern + 독립성 unresolved 없음 | coverage_complete=true | 현재 제한된 후보 평가 범위의 긍정 게이트 통과 |

현재 assessment_metrics는 attendance_pct와 attendance_exception_accepted를 항상 null로 둔다.
따라서 **재선임의 출석 규칙을 완성해 FOR를 내는 입력 경로는 아직 없다.** coverage_complete도
전체 의결권 정책의 완전성을 뜻하지 않으며 현재 신임·독립성 범위만 가리킨다.
...
규칙 자체는 존재하지만 현재 출석 확정 입력이 없으므로 실제 발동 검증 완료로 표현하지 않는다.
...
첫 확장은 회의별 출석 분모와 당해연도 임원 변동, 감사·비감사용역 원문 연결이다.
