...
0.5.0은 임원 변동 공시의 제한된 자동 탐색과 회의·재직·직무정지 평가 입력을 연결한다.
출석은 직전 완료 사업연도 기준으로 서버가 분모·분자를 계산하되 그 입력의 의미·완전성은 LLM 평가이며 사람 미검토다.
공정위·기업집단 원천 연결, 독립 정확성 평가, 운영 배포는 남아 있다.
...
| 출석 평가 완성 | 0.5.0 회의·재직 입력과 산술 검증 연결 | 실제 재선임·직무정지·불참 사례의 원문 귀속 및 완전성을 독립 검증 |
...
사용자는 직전 완료 사업연도 적용을 승인했다. 0.5.0 pilot의 기간을 last_completed_fiscal_year로 변경하며 기본 shadow의 prior_term 정책은 유지한다.

### 출석기간 정책 (0.5.0 파일럿 적용)
...
- 대상 회의 수를 표시한다. 대표성·완전성에 미확인이 있으면 LLM은 unknown으로 제출한다. 최소 표본 수치 문턱은 미정이며 서버가 짧은 재직의 의미를 인증하지 않는다.
...
바꾸는 고객 overlay 입력은 없다. 0.5.0은 회의별 출석 평가 입력을 수용하되 LLM 평가·사람 미검토로 표시한다. 기관 공식 기준이 아니다.
...
이 절은 0.5.0-pilot, opm-llm-assessment/3 코드의 현재 계약이다. 기사·사건의 정형 필드와 자동 갱신은 뒤의 확장 설계다.
...
조회 실패·동일성 충돌·출석 분모 공백은 이 객체로 옮겨 필수 검토를 해제할 수 없다.
...
| 규칙·권고 | not_triggered / fired / excepted / unresolved → FOR / AGAINST / REVIEW | 규칙별 상태를 집계한 뒤 기존 표결 제약 반영 |

수집 상태 이름은 원천별로 다르며 범용 통합 enum이 아니다. 대상이 아닌 안건은 상위 trace에
not_applicable로 표시한다. 적용 조건이 false인 개별 규칙은 현재 not_triggered다.
스키마 오류·중복 task_id는 요청 오류, 다른 과업 ID는 unmatched_task_ids로 미사용 처리한다.
...
2. **원문을 모은다.** 소집공고, 기준일 이전 사업보고서 이사회 절, 지정한 추가 DART/KIND 공시를 읽는다.
3. **과업을 만든다.** 후보 주변 원문·주의사항·정책·출력 스키마를 묶는다. 기존 후보 점수·찬반을 평가 과업에 복사하지 않는다.
4. **호출 LLM이 읽는다.** 후보 귀속·경력·관계·반증을 검토하고 근거 인용과 미확인 사유를 제출한다.
5. **기계가 연결을 검증한다.** 현재 과업인지, 인용이 존재하는지, 타입·개수·허용 키가 맞는지 확인한다.
6. **수용된 평가를 지표로 바꾼다.** 미수용값은 unknown. 출석 관측값을 확정 임기 출석률로 자동 승격하지 않는다.
7. **규칙을 집계한다.** 확정 반대 신호를 먼저 보존하고, 찬성에는 필수 평가 완료를 요구한다.
...
| appointment=renewed, unresolved 없음 | is_reelection=true | 출석 근거 확정 필요 |
| appointment=unknown 또는 미해결 조건 있음 | is_reelection=null | 선임구분 검토 필요 |
...
```text
평가가 수용되지 않음 → 확정 지표 없음 → 후보 권고 REVIEW
반대 규칙 중 하나라도 fired → 후보 권고 AGAINST
그 외에 긍정 게이트 pass AND 미확인 규칙 없음 → 후보 권고 FOR
그 외 → 후보 권고 REVIEW
```

PILOT-ATT는 재선임 여부가 true이고 출석률이 75% 미만이며 예외가 수용되지 않았을 때
반대하는 규칙이다. 75%는 현재 OPM 파일럿 설정이며 기관 공식 기준이 아니다.
출석률은 제출된 회의·재직·직무정지 구간에서 계산한다. 분모·완전성 미확정은 REVIEW이며, 출석 미달의 예외가 unknown이면 반대 규칙도 unresolved다.
PILOT-IND는 independence_concern_accepted=true일 때 반대한다.

### 출석 입력과 원천 탐색의 실행 계약 (0.5.0)
...
- 사업보고서 표지의 사업연도 기간을 읽고 이미 다음 연도도 완료된 오래된 자료,
  날짜 모순, 단축·비정형 사업연도는 자동 확정하지 않는다. 결산기 변경은 별도 검토 대상이다.
...
| 기존 REVIEW + 새 후보 권고 FOR | 부분 평가의 찬성으로 기존 검토 해제 금지 |
...
| 재선임 확인, 위원회 출석률만 있고 이사회 분모 미확정 | 수치를 전용하지 않고 REVIEW | 구현 |
...
이 절 전체는 목표 설계다. 현재 MCP 인자에 아래 객체를 그대로 제출할 수 있다는 뜻이 아니다.
...
| 수용 검증기 | 판단 후보 → 수용된 지표 또는 보류 | 인용 일치 외에 주체·시점·단위·상충 자료 검증을 확장 |
| 결정 엔진 | 고정 정책·수용 지표 → 규칙 trace·권고 | unknown을 보존하는 결정적 집계 및 표결 제약 |
...
- **조정 가능성:** 출석 문턱·평가기간·근거 충족 요건은 이름과 타입이 있는 설정으로 둔다.
  문턱·기간의 고객 수정 허용 범위를 따로 정하고, 변경 전후 영향을 검증한 버전만 활성화한다.
...
현재 회의별 출석·재직 입력과 제한된 임원 변동 원문 탐색을 연결했다. 다음 확장은 당해연도 정기보고서 임원 변동 및 감사·비감사용역 원문 연결이다.
...
정본·현재 정책 JSON·실제 평가 입력 스키마에서 생성한다. 같은 폴더에 정책 JSON, 평가 스키마,
현재 실행 및 목표 갱신 흐름 SVG, 원본 해시를 기록한 manifest가 있다.
