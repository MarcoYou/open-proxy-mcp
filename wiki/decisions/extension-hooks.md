...
## 클라이언트가 모델에게 주는 프리미티브는 다르다 (260907)
Claude.ai 채팅 커넥터는 모델에게 `resources/read` 를 주지 않는다(리소스는 사람이 UI 로 첨부). Claude Code 는 준다.
그래서 확장이 무엇을 노출하든 **도구(tool)가 본선**이고 리소스는 보조다 — 모델이 스스로 부를 수 있는 건 도구뿐이다.
훅이 적는 「원문 위치」 줄도 URI 가 아니라 도구 호출 형태여야 한다. 커넥터는 도구 목록을 접속 시점에 잡아 두므로
새 도구는 재접속 뒤에 보인다.
