...
과거 특정 분기를 지정 조회하는 기능은 없었다. 실사용 세션(예시기업 CW "지난 1년 부문별 매출 추이" 질문)에서
...
연도) 실DART 검증 + 기존 `period` 경로 회귀 없음 확인 + 예시기업 CW 1Q26 rcept_no가 실사용 세션에서 인용된
...
고정 18~24KB window로 잘라내면 해당 소절 뒤의 위험관리·재무자료가 함께 딸려 오고, `MARKDOWN`이 실제 값
존재가 아니라 단순 구간 적중을 뜻하게 된다. 그래서 공개 signature와 markdown-primary 원칙은 유지하되
수집 경계를 구조 헤딩으로 잡는다.
