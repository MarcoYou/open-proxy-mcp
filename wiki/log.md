...
- **왜 이관이 어려웠나가 SDK 가 아니라 테스트에 있었다.** 2.0 은 `mcp.server.fastmcp` 를 제거해
  `MCPServer` 로 바뀌지만 도구 25개·양식 1개의 데코레이터는 무변경이고 wire 도 동일하다. 위험은
  **전송 설정 다섯이 `settings` 를 떠나 `streamable_http_app()` 인자가 된 것**이고, 그중
  `transport_security` 만 조용하다 — 안 넘기면 2.0 은 host 가 localhost 계열일 때만 보호를 켜는데
  OPM 의 bind host 는 `0.0.0.0` 이라 조건에 안 걸려 **보호가 꺼진 채 정상 서빙된다.**
- **완전 다운된 서버 위에서 706/708 이 통과했다.** 기존 스위트는 서비스 레이어라 MCP 층이 통째로
  죽어도 초록이다. 260729 사고(서버 사망·배포 성공·CI 초록)와 같은 모양이라, 이관보다 게이트를 먼저 세웠다.
  - `build_app()` 추출 — 서빙 결정이 전부 `main()` 안에 있어 **테스트가 프로덕션 객체에 닿을 수 없었다.**
  - 계약 게이트 20건. 전부 **부정을 단언**하거나 **실제 wire 바이트**를 본다. 「200 이 나온다」만 재면
    보호가 꺼져도 통과한다 — 꺼진 쪽이 더 잘 통과한다.
  - 적대적 감사로 **변형 57개 중 50개가 초록 통과**하는 걸 확인하고 구멍을 메웠다. 최악은
    「`main()` 이 `build_app()` 을 우회해도 전부 초록」 — 260729 결함이 한 층 위로 옮겨간 자리였다.
- **감사가 live 버그를 찾았다** — `?opendart=%20` 이 200 을 받았다. 파이썬에서 " " 는 참이라 키 게이트를
  통과하고, 하류 폴백도 참이라 **공백이 그대로 DART 키로** 쓰였다. `.strip() or None` 으로 고쳐 배포.
- **판정 기준은 「게이트를 고치지 않고 통과하는가」였다.** 2.0 에서 20건 무수정 통과(추가 1건은 2.0 전용
  위험인 `serverInfo.version`). 고쳐야 통과하면 그건 수정이 아니라 발견이다.
- 배포 후 실측: `tools/list` 55,669B — 원문 차이는 JSON 키 순서뿐이고 정렬 후 완전 동일.
  실제 도구 호출·에러 경로(`"isError":true`)·호스트 보호·키 게이트 전부 정상. 관련: [[mcp-endpoints]].
- 부수: 배포가 테스트를 기다리게(`needs: test`) · 스모크가 빈 서버와 죽은 `/mcp` 를 잡게 ·
  롤백 절차 문서화(flyctl 에 `rollback` 은 없다 — 직전 이미지 재배포) ·
  `scripts/check_branch_against_live.py`(워킹트리를 live 와 대조) 신설.
- **정정**: DART 한도는 IP 가 아니라 **키마다**이고, 분당 초과 시 차단은 24시간이 아니라 실측 2~3시간.
  일일은 키당 4만. 웹 스크래핑(원문·KIND)은 이 한도와 별개이고 공표 수치가 없다.
...
- **2순위 표 1-1-1·1-2-1 — 행 축 파싱 경로 신설.** 항목이 행에, 기수가 열에 놓인 표라 기수 하나를
  ...
  표 1-1-1 「공고일과 주주총회일 사이 기간」은 핵심지표 1번의 근거 수치(최신 연도 28일 이상 82사 ↔ 지표 O 79사).
- **3순위 표 7-1-1·9-1-1·10-2-1** — 이사회 개최·안건통지 간격 / 내부감사기구 구성·재무전문가 표기 /
  외부감사인 소통내역. 핵심지표 13·14번의 근거.
...
- **서식 번호 체계 확정** — 원문은 두 층의 번호를 스스로 단다. 섹션 코드 `[NNNNNN]` 47종이
  ...
  레지스트리 `services/corp_gov_form.py` + [[기업지배구조보고서]] 공시 페이지에 반영.
- **`scope=tables` 신설** — 표 1-2-2(안건별 찬반 주식수) · 4-3-1(이사후보 사전 정보제공기간) ·
  5-2-1(사외이사 겸직) · 7-2-1(개별이사 3개년 출석률·찬성률). 원문 열 이름 그대로, DART 콜 0 증가.
...
- **캐시 확대는 접었다**: 문서 캐시가 `/tmp`(배포하면 사라짐)에 있고 볼륨 `/data` 는 894MB가
  놀고 있어 옮기면 590건까지 살릴 수 있었다. 그런데 **적중률을 모른다** — 텔레메트리에 회사가
  없어(「사용자 조회 결과 저장 안 함」) 셀 방법이 없었다. 게다가 590건이면 사실상 조회 이력이라
  그 원칙과 부딪힌다. 근거 없이 키우지 않기로 했다.
...
- **왜**: 매출 축을 4개(부문·제품·지역·수출/내수)로 재편하면서, 종전엔 세 군데 흩어져 있던
  축들을 한 진입점으로 모았다. 그 과정에서 **지역 축이 조용히 틀린 값을 내고 있던 것**이 드러났다.
- **무엇이 틀렸나**: ① 별도 재무제표 주석을 읽고도 출처 라벨엔 「연결 기준」이 하드코딩돼
...
**원인**: 2026-07-28 `mcp` **2.0.0 정식 릴리즈에서 `mcp/server/fastmcp` 가 통째로 사라졌다**
(실측: 2.0.0 wheel 에 해당 경로 파일 0개 · 1.26.0/1.28.1/1.29.0 은 19개).
`pyproject.toml` 은 `mcp[cli]>=1.6.0` 으로 **상한이 없고**, Dockerfile 은 `pip install .` 로
...
2. `fly.toml` 에 http 헬스체크 — /health 가 200 을 못 내면 fly 가 unhealthy 로 보고 배포 실패
3. 배포 워크플로에 사후 스모크 — 실제 엔드포인트를 10회 재시도로 확인, 실패 시 job 실패

**남은 근본 원인**: Dockerfile 이 `uv.lock` 을 쓰지 않는다. 상한으로 이번 건은 막았지만 다른
의존성에서 같은 일이 난다. Docker 데몬이 있는 환경에서 로컬 빌드 검증 후 처리할 것.

**부수 확인**: fly 로그에 DART 키가 querystring 평문으로 남는다(`/mcp?opendart=…`).
키 교체 + 헤더 이동 과제와 연결된다.
...
- `revenue_mix_form`(II-2-가) · `key_contracts`(II-6-가) 신설 — 영업부문 주석이 「단일부문」이라
  ...
  - `self_check`: 단위·비율합·합계행 대조. **표 단위로** 검산한다 — 구간 통째로는 항목합이 합계행의
    2~3배가 되어 불일치가 절반을 넘는다. 불일치는 오답이 아니라 신호로 남긴다(신풍제약의 라벨 없는
    소계행 이중계상은 비율합 200%로 즉시 드러난다).
- **「해당사항 없음」이 뒤따르는 표를 덮던 결함**: 하이브는 「가. 주요 계약: 해당사항 없습니다」 아래
  「나. 주요 아티스트 전속계약」에 소속사·그룹·아티스트 전원이 있다. 실질 내용 판정이 **숫자 든 행만**
  세는 바람에 이름만 있는 표가 빈 표로 취급돼 4,160자가 통째로 해당없음이 됐다. 엔터사의 핵심 계약이자
  재계약 리스크의 1차 자료다. 이 가드는 9개 필드가 공유하므로 287문서×9필드 전수 회귀를 돌렸다 —
  **전이 10건 전부 NOT_APPLICABLE→MARKDOWN, 소실 0**(두산에너빌리티 NAC International 사용후핵연료
  Cask 기술도입계약 2015~2035, 롯데손해보험 점포현황, 한국앤컴퍼니 판매경로 등).
- **NEEDS_REVIEW를 「해당없음」으로 렌더하며 원문을 버리던 결함**(라이브에서 발견): 남광토건이
  「해당없음 — 」(사유 빈칸)으로 나오고 시공실적 원문 1,507자가 사라졌다. 절을 찾았는데 값만 못 믿는
  상태를 부재로 표기하면 안 된다 — 「원문 · 검토필요」로 분리.
- **`revenue_breakdown` — 매출 분해의 단일 진입점**: 「부문 없음」만 보고 제품 구성을 놓치던 게
  필드가 나뉘어 있어서였다. `by_segment`+`by_product`를 한 필드로 묶되 축마다 `source`(출처·감사여부)를
...
- **소절 상한을 호출 파라미터로**(`section_chars`, 기본 20,000 · 2,000~200,000): 상한을 하드코딩하면
  「잘려서 데이터가 사라진다」가 되지만, 파라미터면 호출측 AI 가 부족할 때 올려서 다시 부른다.
  잘린 응답에 `truncation_note`(올리는 방법)를 넣어 스스로 판단하게 했다. 금융지주가 큰 것은 파서
  결함이 아니라 계열사마다 같은 항목을 싣기 때문이다(KB 재무건전성 소절에 소제목 61개 — 국민은행·
  KB증권·KB손보·KB국민카드·KB라이프…). KB금융지주 145,181→70,188자, 100000 지정 시 191,213자 전량.
- 검증: unit/regression 334 통과(신규 22) · 라이브 MCP 77사 왕복(크래시 0) — 업종분산 30사 스윕
  (반도체·금융지주·보험·증권·건설·조선·제약·엔터·유통) + 파라미터 6조합(format=json · 별칭 3종 ·
  묶음 · 분기보고서) + proxy_advise 내부경로 3사. 스윕에서 ⚠ 6사·145k 응답 1사를 발견해
  전자는 고치고 후자(금융 필드 무상한)는 별건으로 기록.
- 알려진 한계: 기중 종료 계약은 기말 기준 해당없음(세방전지, 286건 중 1건) · L-소절 코드 없는 구형
  문서 48건은 전체 탐색 폴백
...
- `company` (25개 tool 공통 입구)
  ...
  - 역음차 — DART 등록명이 라틴 표기인데 공고 헤더는 한글 음차(「에스케이씨」=SKC). 「엔」은 알파벳 N이자 「엔터테인먼트」의 첫 글자라 길이별 변형을 모두 색인하고, 질의·색인 양쪽에 같은 변환을 건다
  - 업종어 접미(「삼성생명보험」→삼성생명) — 짧게 떼는 것부터. 「생명보험」을 떼면 「미래에셋」이라는 다른 회사가 나온다
  - 못 찾으면 근접 후보를 보여준다 — 앞자르기 자동선택은 오답을 낸다(에스피씨삼립→「케이에스피」·포스코디엑스→「POSCO홀딩스」). 같은 계산을 '제안'으로 쓰면 안전하다
  - 공고 헤더 회사명 322개: 조회 실패 48→22(14.9%→6.8%) · 단일 상장사 248→274 · 복수 후보 26 유지(회귀 0)
- `business_details`
  - 부문 주석 구간을 표준 서식 식별자로 확정, 부문 부재 사유를 근거와 함께 판정('실패' 문면 제거)
- 검증
...
- 미해결(별건)
  - AGAINST 판정 경로 미검증 — 부실기업(감사의견 비적정·완전자본잠식) 표본 확보가 본질
  - 구조화 3건: 자기주식 처분계획(28) · 주주제안 명단 태깅(20) · 주식병합(10) — 원문으로는 나가므로 정보 손실 없음
  - 회사 조회 잔여 6.8%: 스팩 4 · 별칭 · 어중 음차 · 상장폐지·개명 — 유형이 달라 한 규칙으로 덮을 수 없다
  - `coordinate_map` 로드 경로 미검증 — 495건이 전부 '맵 없음' 조건이었다. fly 볼륨에 올릴 때 별도 검증 필요
...
- `geo_revenue` 필드 신설(검산·단위·외부기준 통과분만 정형, 미달 시 원문+사유) + 전 필드 응답 계약 통일(정형→원문→명시적 부재) + 정형 성공 응답에 `self_check` 자가검증 안내 동봉. 시장 표본·라이브 E2E·다중 QA 검증. 상세는 private lessons.

## [2026-07-24] fix+feat | proxy_advise 안건 분류 보강 + 근거 위치 동봉
- 안건 분류에 표준 서식 메타데이터 기반 이중검증 추가 — 제목만으로 분류가 안 되는 안건의 회수와 상충 시 확인 권고(classification_note). 검증 실패 시 해당 절 원문을 통으로 발췌 동봉(표는 마크다운 표 변환).
- 자동 FOR로 새던 중대 안건 유형 봉쇄: 자본감소(무조건 REVIEW·실무 체크리스트)·주식매수선택권(REVIEW)·영업양도/양수(구조개편 합류)·해임(위험 키워드)·주식(액면)병합(자본감소 경로 — 라이브 실사례).
- 상근감사 선임의 법 인용 교정(상법 §542-12④ 의결권 제한·§542-10② 결격 기준, 사외이사 전용 시행령 조항 오적용 제거).
- 각 안건 판단에 근거 위치(source_section — 소집공고 §절 제목) 동봉, 내부 메타데이터는 payload 전용으로 사용자 문면 비노출.
- 검증: 228 tests + 독립 QA·스튜어드십 전문가 이중 리뷰 반영(승격 무표식 금지·바인딩 정합 게이트 등).
...
- **데이터 tool latency pass (2026-05-24)** — `data.timings_ms` 노출 + low-risk 병렬화. treasury_share 결과보고서 검색 전체공시→B/I/E title scan(삼성전자 2.7s→0.9s), dividend metadata overlap, filing_search page2+ 병렬, corp_gov 검색윈도우 4년→2년. median speedup 58.6% (4.137s→0.211s), p95 4.163→0.251s. 핵심: 병렬화보다 "대기를 겹치는 위치"가 중요. (`260510_data_tools_perf_audit`(private 이관) / perf-timing-260524)
- **value_up role extraction (2026-05-31)** — 최신 공시 1개 중심에서 `latest_plan` / `latest_status` / `latest_result` / `meta_amendment` 역할 분리. `계획서 명칭` 기반 보정, meta-only 구간 최근 2년 role backfill, `이행결과` nullable 분리. KOSPI500 + KOSDAQ150, 562 filing 전수조사 기준. (`260530_audit_value-up-implementation-tags`(private 이관) / [[value_up]])
- **financial_metrics Tier 1 확장 (2026-05-31)** — CFO/순이익, DSO/DIO/DPO/CCC 추가. 51 → 56 지표, 추가 API 호출 없음. ([[financial_metrics]])
- **agenda parser marketwide (2026-05-25)** — KOSPI500 + KOSDAQ150, XML 641건, no_filing 9, 3회 재파싱 hash diff 0. 최신 기준은 `260525_1620_audit_agenda-parser-marketwide`(private 이관).
- **agenda relation KOSPI300 rerun (2026-05-25)** — exact 298 / no_filing 2 / requires_review 0, relation metadata는 결론이 아니라 자동 판단을 멈추는 guardrail. (`260525_0200_audit_agenda-relation-kospi300`(private 이관) / agenda-relation-parser-260525)
- **key data tools parsing 성공률 감사 (2026-05-17~18)** — KOSPI 300 + KOSDAQ 150 baseline과 비중복 100개 recheck 기준 문서 신설. 최신 기준은 `260517_parsing_success_rate_audit`(private 이관). `value_up`은 outside-window/013 no_filing 분류 보강 후 strict 100%, `shareholder_meeting_results`는 DART-first 결과 파싱 후 adjusted hard fail 0%.
...
- **파서 전수조사 + 정밀화 검증 (Ralph 5)** — 40 파서 분류 (A 명명형 25 / B raw 보존 1 / C 혼합 14). framework: 데이터 본질에 따라 (숫자→파싱, 자연어→raw, 메타+본문→혼합). parse_aoi_xml이 모범 사례 (clause/label 명명 + before/after raw). audit 1차 권장 (parse_personnel_xml + parse_aoi_xml 보강) Ralph 5 실측 후 무효화 — careerDetails 0% 누락 (44회사/225후보) / aoi 1.66% 누락 (모두 source 한계). 두 파서 정밀도 충분, 코드 변경 X. (`260508_parser_audit`(private 이관) / parser-precision-260508)
- **Wiki 트리 정책 명문화 + lint hook (2026-05-09)** — 식물학 metaphor 도입 (🌱뿌리 raw → 🪵줄기 rules → 🌿큰가지 → 🌾잔가지 → 🍂낙엽). Link 정책: 단방향(위→아래)/양방향(큰가지↔잔가지)/자유(잎↔잎). ABCDE 정리: 단방향 위반 34→0, 양방향 결손 44→0, orphan 24→7, edges 1261→1558. `scripts/wiki_lint.py` + GitHub Actions CI. CLAUDE.md 124→109 가벼움화. 구 *_RULE.md 7개 archive 이동 (`wiki/archive/tools/legacy_rules/`). data-collection.md DS003 섹션 추가 (financial_metrics 4 API). (`260509_wiki_graph_audit`(private 이관) / [[wiki_schema#0-트리-구조-식물학-metaphor]])
...
- 전수조사:
  ...
  - 산출물: `260530_audit_value-up-implementation-tags`(private 이관)
- `financial_metrics`
  - CFO/순이익 비율 추가
...
- 검증:
  ...
  - 테스트: `23 passed`
- 문서:
  - `260525_0200_audit_agenda-relation-kospi300`(private 이관)
