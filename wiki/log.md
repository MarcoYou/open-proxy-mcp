...
- **찾은 방법**: 두 축(`by_segment`·`by_region`)의 총계를 캐시 304건에 걸쳐 대조. 다만
  **일곱 번 재고 여섯 번 틀렸다** — 조정 행 무시 → 프레임 오류 → 호출 인자가 프로덕션과 다름.
  반례 에이전트 2인이 그중 둘을 잡았고, 회계 렌즈 에이전트는 「두 총계가 같아야 한다」는
  전제 자체를 무너뜨렸다(K-IFRS 1108 ¶23은 CODM 측정치·¶33은 재무제표 기초 — 다른 장부).
