...
### 0.0 wiki 와 storage 의 경계

**wiki = 지금 무엇이 참인가(현재형) · storage(private `open-proxy-storage/wiki-private/`) = 어떻게
여기까지 왔나(과거형 — 플로우·실수·레슨런·발견 과정).**

판별식: **그 문장을 지우면 오늘의 동작을 이해하는 데 지장이 있나?** 있으면 wiki, 없으면 storage.

| | wiki | storage |
|---|---|---|
| 예 | 「위원회 표는 번호가 위원회를 특정하지 못한다」 | 「번호로 잡았다가 오탐 나서 제목으로 바꿨다」 |

설계 근거(**왜 이렇게 동작하나**)는 현재형으로 쓸 수 있는 한 wiki 에 남긴다 — 지우면 다음 사람이
되돌린다. **내가 어떤 경로로 틀렸나**는 storage.
`log.md` 는 「무엇이 언제 바뀌었나」(사실)만 적고, 왜 헤맸는지·일반 교훈은 storage lessons 로.
