...
**공개 문서의 이름** — 분석 대상 회사명은 실명으로 쓴다. 리드미·기능 문서·위키의 예시와 사례도 같다(260921).
익명화 대상은 **데이터 출처**다 — 추정치·업종분류 같은 상용 공급처, 의결권 정책·행사 자료를 가져오는 운용사,
비교한 외부 서비스의 이름은 스크립트(`.py` 등)와 PR 제목·설명·커밋 메시지에 쓰지 않는다. 문서에 이미 코드로 적힌
운용사(`K레거시`·`B외국계` 등)는 코드를 유지한다. DART·KRX·법령정보센터 같은 공적 원천과 연결에 필요한 서비스명·명령·주소는
그대로 둔다. 과거 Git 이력은 고치지 않으며 `raw/`·법령 원문·회귀 입력은 치환 대상이 아니다.
