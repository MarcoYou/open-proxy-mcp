...
**공개 문서의 이름** — 고객·내부 업무·분석 사례의 회사명과 브랜드명은 익명 표기로 쓴다. 예시 입력에는
실제 회사명이나 식별자로 바꿔야 한다고 안내한다. 연결에 필요한 서비스명·명령·주소, 법령·공시의 원문 출처는
식별 가능해야 하므로 유지한다. 외부 서비스 비교와 실명 대응표는 private에 둔다. 이름 마스킹은 과거 Git 이력이나
접수번호로 연결되는 공개 원문을 삭제하는 작업이 아니다. `raw/`·법령 원문·회귀 입력은 이 문서 치환 대상이 아니다.
