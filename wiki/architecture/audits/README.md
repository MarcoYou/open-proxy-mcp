...
## audit 산출물 manifest 표준 (260709 패널 검수 — 신규 audit부터 적용)

audit가 `data/`에 남기는 summary.json(또는 동급 manifest)에는 **재현 4요소**를 필수 기록한다.
과거 audit의 "재파싱 3회 hash diff 0" 같은 재현성 주장이 정규화 방법 미기록으로 검증 불가였던
문제(빅데이터 패널 260709)의 재발 방지 — 원시 파일 없이도 재현 가능해야 원시 파일을 지울 수 있다.

1. **생성 스크립트 경로 + git 커밋 SHA** — 어떤 코드가 만들었나
2. **전체 파라미터** — universe 파일·필터·동시성·표본 크기
3. **소스 기준일(as-of)** — DART/KRX는 정정공시로 과거가 바뀌므로 조회 시점 명시
4. **비교·hash 정규화 규칙** — "diff 0" 주장 시 무엇을 정규화(timestamp·순서 등)했는지

버전 관리: `v1..v8`·`phase3/4` 수작업 파일명 접미사 대신 **audit_id(날짜)+커밋 SHA**가 SSOT.
중간 iteration 파일은 폐기하고 최종본만 보존, "왜 v8까지 갔나"는 md 회고에 서술로 남긴다.
