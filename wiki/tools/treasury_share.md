...
- **실행결과보고서 금액 = 표 단위선언 배수 적용(260707 수정)**: `_acode_amount`가 ACODE(예:
  `ACQ_AMT`)를 감싸는 `<TABLE-GROUP>` 안의 최근접 "(단위 : 백만원/천만원/…)" 선언을 찾아 배수를
  곱함 — 원문 표가 항상 원 단위라고 가정하던 기존 `_acode_int`는 최대 100만분의 1로 축소되는
  버그가 있었음(현대차 등 KOSPI200 7개사 60개월치 이력 전량 오염 실측 확인,
  `scripts/treasury_unit_sweep.py`로 재현). 수량류 ACODE(`SUM_ACT_CNT`·`HLD_CNT3` 등)는 이
  배수를 적용하면 안 되므로 그대로 `_acode_int` 유지.
...
- **결정↔실행 "매칭 오탐 4건"의 실제 원인은 금액 파싱오류였음 — 260709 해결·정정.** 260707에 매칭
  로직 문제로 추정했으나, 진단(`scripts/spot_link_cycles_debug.py`) 결과 4건 전부 **날짜 매칭은
  정확**했고 진짜 원인은 금액이었다: ① 포스코퓨처엠 결과금액 3.17천조(`_nearest_table_unit`이 무관
  TABLE-GROUP의 '백만원' 선언을 주워 ×1e6 과대적용), ② 카카오 결정금액 49,850원(파싱오류),
  ③ POSCO홀딩스 4.74배(금액 또는 매칭 오류), ④ 엘앤에프 0.44배는 **정상**(전량 블록딜 저가 처분 —
  오탐). 해결: 주식수(단위배수 미적용이라 정확)로 결과금액 주당 환산가를 검증하는 **금액 sanity 가드**
  — `_sanity_correct_amounts`(주당가 물리불가 시 단위배수 되돌려 자동보정, 포스코퓨처엠) +
  `_flag_cycle_mismatches`(링크 후 비율 >3배·<1% 물리불가 시 `amount_mismatch_ratio` 플래그+경고,
  카카오·POSCO홀딩스). 정상 20개사 24개월 창 오탐 0. 정상 부분집행(0.05~1.5)은 통과.
...
- 2026-07-07: 실행결과보고서 단위(백만원 등) 미인식 버그 수정 — KOSPI200 전수 스캔 26건→0건,
  QA 2인 원문 대조 재검증 통과. 별개 이슈로 결정↔실행 매칭 오탐 4건 발견(TODO).
- 2026-07-09: 위 "매칭 오탐 4건"의 실제 원인이 금액 파싱오류임을 진단·정정. 금액 sanity 가드
  (`_sanity_correct_amounts` 자동보정 + `_flag_cycle_mismatches` 플래그) 추가. 정상 20개사 오탐 0.
