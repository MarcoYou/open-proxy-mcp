...
- **실행결과보고서 금액 = 표 단위선언 배수 적용**: `_acode_amount`가 ACODE(예: `ACQ_AMT`)를 감싸는
  `<TABLE-GROUP>` 안의 최근접 「(단위 : 백만원/천만원/…)」 선언을 찾아 배수를 곱한다. 원문 표가 항상
  원 단위라고 가정하면 금액이 최대 100만분의 1로 축소된다. 수량류 ACODE(`SUM_ACT_CNT`·`HLD_CNT3` 등)는
  이 배수를 적용하면 안 되므로 `_acode_int` 그대로 둔다.
...
- **결정↔실행 사이클의 금액 sanity 가드**: 결정↔실행 비율이 어긋나는 사이클은 **날짜 매칭이 아니라
  금액 파싱**이 원인인 경우가 있다(`_nearest_table_unit`이 무관 TABLE-GROUP 의 '백만원' 선언을 주워
  ×1e6 과대적용). 주식수는 단위배수를 적용하지 않아 정확하므로, 주식수로 결과금액의 주당 환산가를
  검증한다 — `_sanity_correct_amounts`(주당가가 물리적으로 불가능하면 단위배수를 되돌려 자동보정) +
  `_flag_cycle_mismatches`(링크 후 비율 >3배·<1% 면 `amount_mismatch_ratio` 플래그+경고).
  정상 부분집행(0.05~1.5)과 전량 블록딜 저가 처분은 통과시킨다 — 그건 오탐이 아니라 사실이다.
...
- 2026-07-07: 실행결과보고서 금액에 표 단위선언 배수 적용(`_acode_amount`).
- 2026-07-09: 금액 sanity 가드 추가(`_sanity_correct_amounts` 자동보정 + `_flag_cycle_mismatches` 플래그).
- 2026-08-06: 진단 서사를 현재형 설계 근거로 정리(경계 규칙 [[wiki_schema]] 0.0).
