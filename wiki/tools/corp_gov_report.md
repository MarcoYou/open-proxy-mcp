...
- 서식 표 32종 중 11종 노출. 나머지 22종 중 **배당 3종(1-4-1·1-5-1-1·1-5-1-2)은 [[dividend]],
  발행주식 2종(2-1-1-1·2-1-1-2)은 [[ownership_structure]]·[[valuation]], 밸류업 2종(11-1·11-2)은
...
- 2026-07-31: `metrics_summary`에 `prior`·`note`·`note_ref` 추가 — 준수/미준수보다 **사유**가
  ...
  실측(캐시 15건 · 미준수 91개): 실제 설명 69.2% · 포인터 해소 13.2% · 사유 없음 15.4% ·
  표 포인터 미해소 1.1% · 해소 실패 1.1% → **사유 확보 69.2% → 82.4%**. 남은 1.1%는 보고서
  내부 표를 가리켜 표 파싱이 필요하고, 15.4%는 회사가 사유를 안 적은 것이라 파싱으로 못 푼다.
  소비처는 [[proxy_advise_before_meeting]]의 `governance_non_compliant`.
- 2026-08-05: `scope=tables` 신설 — 서식 표 4종(표 1-2-2 안건별 찬반 주식수 · 4-3-1 이사후보 사전
  정보제공기간 · 5-2-1 사외이사 겸직 · 7-2-1 개별이사 3개년 출석률·찬성률)을 원문 열 이름 그대로
  낸다. 이미 받아 둔 원문에서 뽑아 DART 콜 0 증가. 표 번호·개념 코드 레지스트리는 `services/corp_gov_form.py`.
- 2026-08-05: **금융회사가 몇 해 전 보고서를 가리키던 것 수정** — 보고서명에 「연차보고서」가 있으면
  걸러내고 있어, 연차보고서로 갈음하는 금융회사는 그해 공시가 통째로 사라졌다(KB금융 2024-02-29을
  최신으로 표시 → 2026-03-05). 이름 필터를 걷고 같은 해에 거래소 서식이 함께 있을 때만 미룬다.
- 2026-08-05: 금융회사 안내 문구 정비 — 「미제출이 아니라 서식이 다르다」와 면제 근거(연차보고서를
  기한 내 공시하면 거래소 서식 제출 면제)를 밝히고 첨부 PDF로 안내한다. 렌더가 「핵심지표를 읽지
  못했습니다」로 내려가 파싱 실패로 읽히던 것도 함께 고쳤다.
- 2026-08-05: `filings_count` → `filings_found` 개명 (`filing_count` 와 혼동 방지).
- 2026-08-05: `scope=tables` 4종 → **10종**. 주총 운영 3종(1-1-1·1-2-1·1-2-2) · 이사 5종(4-2-1·
  4-3-1·5-2-1·7-1-1·7-2-1) · 감사 2종(9-1-1·10-2-1). 전수 249/249 · 누적 13,708행 · DART 콜 0 증가.
  1-1-1·1-2-1 을 위해 **행 축 파싱 경로** 신설(기수 하나 = 한 줄로 전치, `rowspan` 은 여기서
  진짜 병합이라 부모·자식 라벨을 이어 붙임). 표 1-1-1 의 「공고일과 주주총회일 사이 기간」은
  핵심지표 1번(4주 전 소집공고)의 근거 수치다 — 최신 연도 기준 28일 이상 82사로 지표 O 79사와 맞는다.
- 2026-08-06: 표 4-1-2(이사회 구성 현황) 추가 — 10종 → 11종. 「전문 분야」·「주요 경력」은 다른
  경로에 없고 채움률이 높아 이사 후보 적격성 판단에 바로 쓰인다(`director_board` 의
  `main_career` 는 사업보고서 기반이라 축이 다르다). 라이브 표본 25사 전부 추출, 선두 열
  211개 값 중 4개(외국인·자간 패딩)를 뺀 나머지가 사람 이름이라 `이사` 로 이름을 붙였다.
