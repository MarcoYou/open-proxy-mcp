...
**성립 조건**: 이 등식은 `fin_year = target_year - 2`에 의존한다. 주총 N년 → 안건은 FY(N-1) 승인,
분석 reference는 FY(N-2)(작년 주총에서 이미 확정된 안정 데이터). 본문의 전기도 FY(N-2)라 같은 해가
되고, 소집공고 시점에 API는 그 FY(N-2)를 이미 1년째 갖고 있다. reference 선택이 「최신 사업보고서」로
바뀌면 제출 전후로 API가 FY(N-1)로 옮겨가 검산이 조용히 무너진다 — 소스 계약 테스트로 잡는다.
...
- `_decide_financial_statements` — **감사의견 축과 자본잠식 축을 한 문장에 뭉치지 않는다.** 뭉쳐 있던
  ...
  - **잠식률 산정 기준(연결 자본총계 vs 지배지분)은 아직 확정하지 않았다.** 회계 검토는 「비지배지분
    제외가 실무 통설」이라 했으나 조문 원문 미확인이고, 웹 확인에서는 「자기자본 = 자산총액 − 부채총액」
    (= 비지배 포함)이라는 서술과 「지주·지배회사는 지배지분 기준」이라는 서술이 **서로 엇갈렸다**.
    1차 자료로 확정하기 전에는 **바꾸지 않는다** — 반대표 근거를 불확실한 규정 해석 위에 세우지 않는다.
