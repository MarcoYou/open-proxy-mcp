...
## 260730 — 안건 제목 뭉침으로 roleType 이 오배정되던 것 (수정 완료)
...
`declared_role_for_candidate(title, name)` 신설 — **이름 바로 앞의 직위**를 먼저 본다.
근거를 `named`(이름 지목) / `sole`(제목에 직위 하나뿐) / 없음으로 구분하고,
직위가 둘 이상인데 지목이 없으면 **판단하지 않는다**(추측하면 그게 버그다).
`roleType` 에도 출처(`roleTypeBasis` = `column` / `title` / `title_named`)를 남긴다.

**덮는 경우는 하나뿐**: roleType 이 구간 제목 추정(`title`)이거나 없는데(`none`),
제목이 이 후보를 이름으로 지목한(`named`) 경우. 후보자 표 컬럼(`column`)에서 온 값은
후보별 정형이라 덮지 않고 `roleTypeConflict` 로 충돌만 남긴다 — 덮으면 세분도 차이까지
함께 깨진다(앞선 세션 측정 106건).

전수 회귀(캐시 소집공고 후보 1,399명):

| | 인원 |
|---|---|
| 교정 — 감독→비감독(독립성 오적용 해소) | **13** |
| 교정 — 세분도만 변경 | 3 |
| 충돌만 표시(컬럼 유지) | 12 |
| **비감독→감독(새 독립성 검증 추가)** | **0** — 검증을 새로 붙이는 방향은 없다 |

판정 회귀(5사·49안건, production 렌더 경로): **찬성/반대/검토 변화 0건.** 사유와 근거만 바뀐다.
오염 대조 경보 0/10(숫자 소실 0 · 표 행 감소 0 · 크래시 0).

대표 사례 — **대웅제약 박성수**(현직 대표이사):

```
전: 「사외이사」 → 독립성 평가 적용 → 「최근 2년 직원 이력: 외부인」
    (근거란에는 「2015 ~ 현재 (주)대웅제약 대표이사」가 그대로 적혀 있었다)
    사유: 「사외이사 결격 없음」 · 사내이사 성과 평가 누락
후: 「사내이사」 → 독립성 평가 비대상 · 재직 성과 양호(4/12, 2013~2026)
    이사회 구성도 교정: 사외/독립 5 + 사내 0 → 사외/독립 3 + 사내 2
```

`names_titled_inside_director` 는 §382③ 검산용으로 유지한다(공고 전체 제목 스캔).
...
- 2026-07-30(2차): **안 쓰던 임원현황 필드 3종 활용** — 전부 추가 DART 콜 0.
  - **직위 표기 불일치 노출**(`role_type_conflict`): 후보자 표와 안건 제목이 직위를 다르게
    밝히면 어느 쪽도 덮지 않고 그 사실을 후보 블록에 찍는다. 직위가 갈리면 독립성 검증 적용
    여부가 달라지므로 사람이 원문을 봐야 한다. 실측 3명(거짓 충돌 9건은 `sole` 과잉적용
    수정으로 사라졌다).
  - **임기 만료일**(`tenure_end_on`, 등기 96.3% 채움): 서식이 「YYYY년 MM월 DD일」 하나라
    `hffc_pd`(재직기간)보다 해석 여지가 없다. ① 만료가 이번 회차면 **재선임 대상 확정**
    (실측 71명 중 37명 = 52.1%) — 경력 텍스트 추론과 독립된 정형 근거이므로 `type=new` 를
    연임으로 승격한다(승격만). ② 생년이 없어 취임연령 게이트를 못 돌릴 때
    (만료 − 시작) > 40년이면 근속 오기재로 보고 시작연도를 버린다. 산출물에 「임기 만료: …」로
    노출한다 — 「이 사람 임기가 언제 끝나나」는 스튜어드십 실무의 기본 정보다.
  - **이사회 성별 구성**(`sexdstn`, 100% 채움) → **자본시장법 §165조의20**: 자산총액 2조원 이상
    상장사는 이사회를 특정 성의 이사로만 구성할 수 없다(2020 개정 · 2022-08-05 시행).
    실측 30사 중 **10사가 등기이사 여성 0명**이고, 자산 2조를 넘는 회사에서만 위험 신호를 낸다
    (유안타증권 발동 / DB 1.0조·한컴라이프케어 0.2조 미발동). **등기이사만 센다** — 미등기
    여성 임원을 이사회 여성으로 세면 위반을 놓친다. 판정은 하지 않는다(자산은 FY(N-2) 기준,
    이사회 구성도 스냅샷).

- 2026-07-30(3차): **담당업무·지배구조 핵심지표** — 여기서 방향이 한 번 바뀌었다.
  `chrg_job`(담당업무, 95.8% 채움)에서 「대표이사 겸 이사회 의장」을 뽑으려 했는데,
  `corp_gov_report` 의 **15개 핵심지표에 이미 「사외이사가 이사회 의장인지 여부」가 있었다**.
  기업지배구조보고서가 더 권위 있는 소스이고 `chrg_job` 의 의장 식별률은 4.7%뿐이라
  (표기 없음이 「분리됨」을 뜻하지 않는다) **중복 구현을 접었다.**
  - 대신 **미준수(X) 지표를 산출물에 실었다**(`governance_non_compliant`). 종전엔 준수율
    (「93.3% · 14/15」)만 보여줘 어느 지표가 빠졌는지 알 수 없었다 — 그중엔 의결권 판단에
    바로 닿는 것이 있다(실측 대웅제약: **「사외이사가 이사회 의장인지 여부」 미준수**,
    「집중투표제 채택」 미준수).
  - **§165조의20 신호가 교차검증됐다**: 15개 지표에 「이사회 구성원 모두 단일성(性)이 아님」이
    있다. roster `sexdstn` 집계와 대조(5사) — 여성 0명 ↔ 「X」, 여성 있음 ↔ 「O」, **불일치 0건**.
    두 소스가 독립적으로 같은 말을 한다. 유안타증권은 지배구조보고서가 없어 정형만 있고,
    그 빈틈을 `sexdstn` 신호가 메운다.
  - `chrg_job` 에서 **위원회 소속**은 살렸다(`roster_committees`) — 감사위원회 39 · 내부거래 20 ·
    ESG 20 · 사외이사후보추천 14건 언급(등기 190명). 감사위원 후보가 직전 보고서에 이미 그
    위원회 소속이면 연임, 아니면 신임이라 판정 근거가 된다. 원문에 적힌 것만 읽고 추정하지 않는다.
  파일럿(5사·71안건): **판정 변화 0건** · 오염 경보 0/10 · 추가만(사라진 줄 없음).
- 2026-07-30(4차, 마무리): 임원현황 **유휴 필드를 다 썼다**.
  - `ofcps`(직위, 100%) → `roster_position`. 소집공고 「주된직업」은 자유기재라 다른 회사
    직책일 수 있는데 `ofcps` 는 **이 회사 현재 직위**를 정형으로 적는다. 실측 53명 중
    **43명(81.1%)** 에서 주된직업에 없는 정보를 담고 있었다(삼성전자 전영현 — 정형 「부회장」).
  - `main_career`(주요경력, 98.6%) → `career_from_roster`. 소집공고 세부경력이 **없을 때만**
    메우고 덮지 않는다 — 기준일이 다르다. 실측 결측 1.9%지만 하드케이스다
    (모나리자 Lok Shean Yang Peter — 외국인명).
  - **회장·부회장 → 총수 일가 신호는 하지 않았다.** 직위는 11.3%에서 잡히지만 짝지을
    `mxmm_shrholdr_relate` 가 「계열회사 임원」 boilerplate 라(삼성전자 독립 사외이사 전원
    동일값) 조합이 성립하지 않는다 — 기존 코드가 그 값을 non-meaningful 로 분류해 둔 이유다.
  파일럿(5사·66안건): 판정 변화 0건 · 오염 경보 0/10 · 모나리자 +319자(경력 결측 보완).

**임원현황 8필드 활용 현황** — 전부 추가 DART 콜 0:

| 필드 | 채움 | 용도 |
|---|---|---|
| `rgist_exctv_at` 등기 구분 | 100% | 등기 여부 확정 · 미등기면 성과 미평가 |
| `hffc_pd` 재직기간 | 96% | 등기 시작연도(취임연령·임기만료 정합성 게이트) |
| `tenure_end_on` 임기 만료일 | 등기 96.3% | 재선임 대상 확정(52.1%) · 근속 오기재 검산 |
| `fte_at` 상근 여부 | 100% | 사외이사 §382③1호 결격 검산 |
| `sexdstn` 성별 | 100% | 자본시장법 §165조의20(지배구조 지표와 교차검증) |
| `chrg_job` 담당업무 | 98.6% | 위원회 소속 |
| `ofcps` 직위 | 100% | 이 회사 현재 직위(주된직업과 별개) |
| `main_career` 주요경력 | 98.6% | 공고 경력 결측 보완 |
| `mxmm_shrholdr_relate` | 51% | 최대주주 관계 결측 채움(boilerplate 제외) |

- 2026-07-31: **「잘 쪼갰나」를 의심하고, 의심되면 원문 두 칸을 함께 싣는다.**
  짝이 맞아 보여도 잘 쪼갰다는 보장은 없다 — 소집공고 후보표는 서식이 회사마다 달라
  기간·내용 셀의 줄 구분(`<p>`)이 있기도 없기도 하다.
  - `career_split_doubt` 3신호 — ① 기간이 항목별로 갈리지 않음 ② 한 항목에 회사가
    여럿(뭉침) ③ 항목이 괄호 중간에서 잘림. 하나라도 뜨면 쪼갠 목록에 **원문 두 칸을
    나란히** 붙여 대조하게 한다. 2026 소집공고 캐시 41건·후보 189명 실측 **11.1%**
    (뭉침 7.9% · 기간 미분할 2.6% · 절단 0.5%).
  - 기간이 항목별로 안 갈리면 **짝짓지 않는다** — 파서가 기간 하나를 전 항목에 복사해
    없는 기간을 만들던 것(카카오 정신아: 원문 시점 8개가 「1997 ~ 현재」 6회 반복으로).
  - `_split_company_role`·`_build_career_company_groups` 폐기(-56줄). 원문을 통째로
    주는 마당에 회사/직위를 갈라 붙이는 정규화는 매칭 기여가 0.39%였다.
  - **원문 저장을 무조건으로.** `<p>`가 아예 없는 순수 텍스트 셀은 파서의 앞 분기에서
    리턴해 원문 저장 지점에 닿지 못했고, 그래서 의심만 띄우고 보여줄 원문이 없는
    빈손이 생겼다(신한지주 진옥동 — 기간 4구간이 2항목으로 뭉치고 「ㆍ신한은행
    은행장ㆍ(」에서 잘려 「(現) 신한금융지주회사 회장」이 통째로 사라짐).
  - `not_evaluated` 사유를 4갈래로 — 「미등기 확정」/「등기는 확정인데 시작 시점 미상」/
    「임원현황에 없음」/「매칭 실패」. 등기 여부를 아는데 「소집공고 경력란은 등기 여부를
    적을 의무가 없습니다」라고 답하던 것(대웅제약 박은경: `director_type` 사내이사 ·
    임기만료일 2027-03-28이 있는데도). 미확정 45명 분포 = 13.3% / 6.7% / 80.0%.
  · 검증(캐시 41건·후보 200명, DART 0콜): 원문 저장 무조건화 전후로 **파싱 결과 변화
  0명 · 의심 판정 변화 0명 · 기존 원문 변화 0명 · 원문 잃음 0명**, 원문 확보 46.0% →
  54.5%(순수 추가 17명), 의심 발동 21명 중 빈손 **1명 → 0명**. 파일럿 오염 0/8 ·
  판정 61안건 변화 0 · 라이브 MCP(고려아연·카카오·대웅제약·대한제분) 확인.

- 2026-07-31(2차): **원문 폴백을 정형 소스 두 곳에도 — 확정 못 했으면 근거를 통째로.**
  소집공고 쪽엔 폴백이 갖춰져 있었는데(안건·경력·추천사유·직무계획) **임원현황과
  기업지배구조보고서**엔 없었다. 판정만 내보내고 그 판정의 근거 행은 payload에 없었다.
  - `performance.roster_row` — 등기 시작을 **확정하지 못한 갈래에만** 임원현황 행을 통째로
    (등기 구분·직위·담당업무·재직기간·임기만료일·상근·최대주주 관계·주요경력, 빈 칸 제외).
    대웅제약 박은경은 정형이 「사내이사 · 임기만료 2027-03-28」로 등기를 확정해 주는데
    `재직기간 2010.1 ~現`이 취임연령 게이트에 걸려 시작연도가 미채택됐다 — 사유 문장은
    나가는데 걸린 그 표기를 볼 방법이 없었다. **확정된 경우엔 붙이지 않는다**(군더더기).
  - `governance_non_compliant`를 라벨 목록 → `{label, note, note_ref, prior}`로. 회사가 적은
    **사유**와 전년 값을 함께 싣는다. 「사외이사가 이사회 의장인지 여부 X」만 보면 왜인지
    모르는데 회사는 「사내이사가 이사회 의장직 수행」이라 밝혀 둔다. 전년 값이 있으면
    이번에 나빠진 것인지 계속 그랬던 것인지도 갈린다.
  - **포인터를 따라간다** — 비고를 「(세부원칙 4-1) 참고」로만 채우는 서식이 있다(캐시 1건은
    15/15). 그 회사는 사유가 통째로 빈 것과 같았다. 세부원칙은 같은 문서에서 이미 파싱되므로
    **추가 DART 콜 0**. 원문 note는 그대로 두고 `note_ref`로 따로 붙인다. 표(「표 1-1-1 참고」)는
    해소하지 않는다 — 가리키는 대상이 원칙이 아니다.
  · 실측(지배구조보고서 캐시 15건 · 미준수 지표 91개): 실제 설명 69.2% · 세부원칙 포인터
  해소 13.2% · 사유 없음(회사가 안 적음) 15.4% · **표 포인터 미해소 1.1%** · 세부원칙 해소
  실패 1.1% → **사유를 볼 수 있는 비율 69.2% → 82.4%**.
  · 남은 1.1%는 「표 1-4-1: 배당기준일과 배당액 확정일 참고」처럼 **보고서 내부 표**를 가리켜
  표 파싱이 있어야 풀린다. 15.4%는 파싱 문제가 아니라 회사가 사유를 안 적은 것이라 더 파싱해도
  안 나온다 — 지배구조 쪽 파싱 실익은 여기서 거의 소진됐다.
  · ⚠ 분류 함정: 「표 X-Y 참고」는 `세부원칙` 문자열이 없어 최초 집계에서 **「실제 설명」에
  섞여 들어갔다**(68.2%로 과대). 포인터 판정을 문자열 하나로 하면 서식이 다른 포인터를 놓친다.
  · 회귀(8사 before/after): **안건 판정 변화 0 · 성과 분류 변화 0**. 출력 증가는 근거가 붙은
  곳뿐이고(모나리자 +1,337자 · 대웅제약 +639자) 확정된 회사는 +0자(고려아연·신한지주).
...
- 2026-08-02: **추천사유가 남의 것으로 붙던 것 — 한 구간 안에서 후보별로 가른다.**
  260727에 하위안건 단위로는 갈랐지만, **한 구간에 후보 전원의 사유를 담는 서식**(실측 다수)에서는
  여전히 첫 후보의 문면이 전원에게 붙었다. 코오롱글로벌 6명이 모두 「1. 김영범본 후보자는…」을
  달고 나갔고, 그것도 `shared=False`(이 후보 것이 확실함)로 표시됐다. 검사 출신 후보에게
  스탠퍼드 경영학 박사 이력이 붙은 건도 있었다. 사유는 `recommendation_reason_raw` 로 LLM
  판단에 그대로 들어가므로 **남의 이력으로 이사를 판단**하게 된다.
  → 공고가 「- 홍길동 후보자」·「[홍길동 후보자]」·「5. 김학진」처럼 **이름으로 구간을 스스로
  선언**하므로 그 선언을 읽어 가른다(선언 구조 파싱 — 표지 형태는 캐시 소집공고 411사 실측).
  캐시 47명 기준 **22명(47%)의 사유 귀속이 교정**됐고, 본인 이름으로 시작하는 사유가 16→29명.
  ...
  - **파일럿이 잡은 회귀**: 한 단락에 본인 이름이 두 번 나오는 서식(LG화학 김동춘)에서 구간을
    쪼갠 뒤 `"\n"` 으로 되붙여, 내용은 같은데 md 렌더의 `- 추천 사유:` 불릿이 끊겼다.
    연속 동일이름 표지는 한 구간으로 병합해 해결(인위적 줄바꿈 41명 중 2 → 0).
    **테스트 452개도 캐시 전수도 못 잡았다 — 둘 다 텍스트만 봤지 렌더를 안 봤다.**
