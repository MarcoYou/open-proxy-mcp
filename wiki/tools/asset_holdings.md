...
> "예시기업 EL 보유 자산 좀 봐줘, 시총보다 많이 갖고 있어?"
>
> "이 회사 상장 자회사 지분 시가로 다시 계산하면 얼마야?"
>
> "예시기업 FO 자산 구성이랑 시총 대비 잉여자산 비율 알려줘"
...
- 예: `asset_holdings("예시기업 EL")` → 지주사 숨은 지분가치(예시기업 FY 장부 820억→시가 7,736억,
  미실현 +6,916억) · `asset_holdings("예시기업 FO", scope="detail")` → 소규모기업(OFS) 원문 명세.
...
(예시기업 BP=USD). 환산하지 않으면 **분자 USD ÷ 분모 KRW** 가 되어 배수가 환율배만큼 축소된다 —
...
- 2026-09-08: **기능통화 환산**(`functional_currency`·`fx_rate_to_krw` 신설). 종전엔 USD 장부가를
  KRW 시총으로 나눠 배수가 약 1,400배 축소됐다(예시기업 BP 잉여자산배수 0.00 → 0.346). 세부 계정 표를
