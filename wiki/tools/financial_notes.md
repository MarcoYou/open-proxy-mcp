...
| 코드 | 구간 | 쓰임 |
|---|---|---|
| `D-0-3-2-0` | 2. 연결재무제표 | 뺄셈의 **분모**(재무상태표) |
...
- **🏷 계정 + 재무상태표 잔액** — 사용제한액을 **어느 계정에서** 뺄지와 그 분모.
  계정명이 **완전일치**할 때만 붙인다. 아니면 「분모 없음」을 명시한다.
...
- 은행 「예치금」은 재무상태표에 그 이름의 계정이 없어 분모가 안 붙는다(상위 계정에 묶여 있다).
