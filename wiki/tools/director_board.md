...
| scope | 이럴 때 물어보세요 |
|---|---|
| `compensation` | "등기이사 인당 보수 얼마야?" · "예시기업 HM 이사 보수한도 소진율 얼마야?" · "예시기업 CW 최근 3년 한도 다 썼어?" · "예시기업 K 한도초과 이유가 뭐야?"(→ rm 비고로 퇴직금 등 사유 확인) |
| `roster` | "예시기업 AV 이사회 구성원 누구누구야?" · "예시기업 HM 작년에 이사 누가 오고 누가 나갔어?" · "이 회사 사외이사 중에 최대주주랑 관계있는 사람 있어?" |
| `individual` | "예시기업 CW 대표이사들 각각 얼마 받아?" · "이 회사 등기이사 중에 스톡옵션·RSA 받은 사람 있어?" · "정의선 작년 대비 보수 얼마나 늘었어?" |
| `unregistered` | "예시기업 HM 미등기임원 평균 보수 얼마야?" · "등기이사랑 미등기임원 보수 차이 얼마나 나?" |
...
| `pay_criteria` | "이 회사 상여는 무슨 기준으로 줘?" · "예시기업 AB 성과급 KPI 가중치 뭐야?" · "대표이사 급여/상여 어떻게 산정했어?"(사업보고서 VIII-2 원문, summary 제외 on-demand) |
...
- 실지급은 유형별 여러 행(등기이사(사외·감사위 제외)/사외이사/감사위원 등). **감사위원회 위원은
  이사 한도 안**(등기이사이므로) — IR 확증: 예시기업 HM 한도 12명 = 실지급 버킷 5+2+5 헤드카운트와 정확히
...
- **한도 행은 합산한다.** 이사 한도를 상임/비상임 또는 등기/사외로 **분리 공시**하는 회사가 있어
  (한국전력·예시기업 AW·예시기업 AP 등), 마지막 행으로 덮으면 한도가 5~8배 축소돼 소진율이 허위로 폭증한다.
...
깨지기 쉽다). 내용 예: 퇴직금·중도사임 등 1회성 사유(예시기업 N "퇴임한 사내이사의 퇴직소득(7억원) 포함"
...
```
예시기업 HM 실측(2024): 등기이사 32.0억 ÷ 직원 1.24억 ≈ **25.8배**. 배수 자체가 과다/적정 판단은 아님
(업종·직군 구성 차이) — 비교 신호로만.
...
작년 소진율 = 작년 실지급 ÷ 작년 한도
```
예시기업 HM 실측: 제안 284억 / 작년 237억(인상 +19.8%) / 작년 실지급 236.9억(**작년 소진율 100%**) →
...
`auto`(임박 회차 우선)로 고르면 보수한도 안건이 없는 임시 공고를 읽는다(2026-09-04 실측 예시기업 AQ:
09-09 임시주총 공고가 최신이라 3월 정기 제6호 120억 한도를 놓치고 `no_agenda`. 예시기업 DZ도 동일).
...
- **attendance 부분성**: 회사가 개별 출석률을 '(출석률:%)' 인라인으로 요약하는 범위가 제각각 —
  **일부(주로 사외이사)만** 그 형식으로 쓰고 나머지는 회차별 출석표에만 있는 회사가 많다(예시기업 AV 4/9명).
  parsed 인원<이사회 인원이면 `attendance_partial` flag. 전체 이사회 출석을 다 잡으려면 회차별
  매트릭스 파싱이 필요(TODO). 표 자체가 없는 회사(예시기업 DH·예시기업 DY 등)는 `not_found`.
  겸직(표5-2-1)·선임변동은 미구현.
- **birth_ym 결측 시** 보조키로 `hffc_pd`(재직시작일) 검토 — TODO.
- **개인별 보수 5억 미만 비공개**: `hmvAuditIndvdlBySttus`는 상위 일부만(범주 평균은 전원).
- **지연 제출 rcept**: 통상 3월 기한보다 늦은 rcept_no(예 예시기업 CD 2024=8월)는 정정공시 가능성 —
  버킷 내부 정합 확인 권장.
- **미해결 각주 마커**: 승인한도 rm·개인별 breakdown에 원문 각주 마커(`(*1)`·`주1)`)가 그대로
  남는 회사 있음(원문 각주 본문과 분리 제공) — 마커만 노출돼 의미 없는 경우 있음. 원문 각주 연결 TODO.
- **지주사 pay_gap 비교**: 금융지주·순수지주 격차배수는 지주사 본사 인력(예시금융지주 A 195명·예시금융지주 D 144명) 기준이라
  사업회사(예시기업 CW 54배)와 직접 비교 부적절 — 값은 정확하나 회사간 비교 시 주의(캡션 보강 TODO).
...
실측: 예시기업 HM `lookback_years=3` 기준 11.9초 → 3.0초.
...
- **section-local 필수**: 같은 `주1)`이 임원보수·재무각주·종속기업에 **따로 존재·의미 다름**(예시기업 GC 실측)
...
**무엇을 뽑나**
- **보수지급기준(정책)**: 등기이사/사외이사/감사위원 버킷별 급여·상여·단기/장기성과급 배수(예시기업 FW 단기
  0~280%(사장)/0~165%(사내), 장기 0~600%, RSU 3년 이연 등).
- **개인별 산정기준**: 실명 임원의 급여/상여/주식보상 분해 + 계량·비계량 KPI(예시기업 AB 영업이익 15%·매출
...
- **이중공시 scope 처리**: 같은 사람이 「이사·감사 개인별」과 「상위 5명 개인별」 **두 법정표에 다른
  금액**으로 동시 공시될 수 있다(예시기업 FH 김태해: 이사·감사 956M=등기 자격분 vs 상위5명 1,286M=연간
...
실측: 예시기업 Y `year=2026` → 「2026년 1분기보고서」 기준 121명.
...
실측(예시기업 CW 2025 사업보고서 → 2026 1분기): 김용관 신규 사내이사 · 송재혁·유명희 이탈 —
...
- 2026-09-07: **속도 — 예시지주 A 7.8초 → 2.7~3.8초, 이벤트 루프 최대 멈춤 4~6초 → 0.4초.** 프로파일 결과 시간의 70%가 `pay_agenda` 가 부르는 소집공고 파싱(2.9~5.4MB, bs4)이었고 전부 동기라 루프를 잡았다. 파싱을 워커 스레드로(`asyncio.to_thread`), scope 가 안 쓰는 임원 표는 건너뛰고, 후보 분류 때 파싱한 meeting_info 를 번들이 재사용. `tests/test_notice_parse_offload.py`.
