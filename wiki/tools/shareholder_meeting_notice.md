...
- 인라인 하위안건 분리 + 표/변형번호 흡수 차단 + 군더더기 부호 정리 (2026-06-21, 891건 html 경로 실측 잔여 손상 교정 — 순수함수, DART 0콜):
  ...
  - **안건 빈 제목 부모 추론** (2026-06-21, 통합 3,016 전수) — 부모 `제N호`가 제목 없이 `제N-M호` 하위로만 시작하는 형태(국영지앤엠형이지만 `제N-M호` 별도노드형: 파인디앤씨 제2호·스튜디오드래곤 제3호·파버나인 제3호). `_build_tree` 직후 `_fill_empty_parent_titles`로 하위 공통 안건유형(`이사/감사`+`선임/중임의 건`) 추론. 빈제목 10→3(잎노드 자체가 빈 3건 — 코렌텍·이삭·티에스넥스젠 — 은 후속). regression 0.
  - **(후속) bleed 신규 표현** — `[#붙임 약력]`·`Ⅳ.전자투표`·`4.참고사항`·`의안순번` 등은 `_AGENDA_BOUNDARY` 표현 추가가 `AGENDA_RE` 제목 캡처(non-greedy)와 들쭉날쭉이라 보류. 근본 해결은 표현 나열이 아니라 `_clean_title` **어미 기반 절단**(`…의 건/승인/선임` 뒤에 비안건 신호 `[`·`N.`·로마숫자·`순번` 등이 오면 절단)으로, 단 `[현금배당]` 부속 보존을 전수 검증해야 하므로 별도 진행.
