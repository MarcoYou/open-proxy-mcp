...
> "예시기업 CW·예시기업 AL 자사주 숫자까지" → `types="treasury"`, `universe="custom:예시기업 CW,예시기업 AL"`, `details=true` (코드/이름 혼용 가능).
...
- **universe**(2026-07-15 시장 분리 + 이름 해석): **all**(디폴트 전체시장) / `kospi200`(=KOSPI 시총상위200,
  **코스닥 미포함** — 지수 원장 부재라 시총상위 대체) / `kospi:N` · `kosdaq:N`(시장별 시총상위) /
  ...
  `custom:종목,종목`(**코드 또는 회사명 혼용** — "예시기업 CW" 같은 이름은 `resolve_company_query`로 자동 코드화,
...
- **자회사 계약 재공시**(「…(자회사의 주요경영사항)」): 같은 날 같은 금액으로 자회사가 직접 낸 공시가 있으면 중복으로 빼고, 없으면(비상장 자회사) 유일한 기록이라 건수에 넣는다. 그 비율은 **자회사 매출 기준**이라 모회사 평균·누적 합에서 뺀다(큰 수주 목록엔 「자회사 계약」 표시). 실측: 예시기업 CL가 예시기업 CK 계약을 같은 금액·같은 89.8%로 다시 공시.
...
| "예시기업 CW, 예시기업 AL" | `custom:…` (이름→코드 변환은 거기서 이미 한다) |
...
- 검증(2026-07-15): scan 라이브(전체시장 하루 172건→91포착, 4콜) · details 6/6 파싱(예시기업 CW 자사주 3,228억 시총대비·예시기업 CQ 19.7% 경영참여·예시기업 AW DPS 210원·예시기업 GR 임시주총 안건·예시기업 AL 유상증자·수주 매출대비%).
