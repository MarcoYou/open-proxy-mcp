...
- 2026-07-21: capability 안내용 `getting_started` 도구를 검토·추가했으나 같은 날 제거했다. 서버 `instructions`만 유지하며 현재 카탈로그에는 포함하지 않는다([[260721_1600_decision_getting-started-tool-vs-resource]]).
...
- 2026-08-03: **business_details·asset_holdings 응답 계약 확장** — 값이 없을 때 「해당없음」 한 갈래로 뭉뚱그리던 것을 `absence_kind` 넷(`not_disclosed`·`cross_reference`·`narrative_only`·`extraction_failed`)으로 가르고 `absence_note`에 근거를 실었다. 값이 있을 때는 **원문 위치**(그 회사의 소절 제목 / 주석 원문 문구 인용)를 함께 낸다. `asset_holdings` 주석 4필드에는 **연결/별도 기준**(`basis`)과 불일치 경고(`basis_conflict`)가 새로 붙는다 — 문서가 셀마다 선언한 XBRL 컨텍스트를 읽으며, 선언이 없으면 기준을 내지 않는다. 앵커·게이트 정확도도 함께 개선(회귀 0).
