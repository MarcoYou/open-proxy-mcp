...
- **결정공시 횟수는 코스피·FY2020~2024 만.** 코스닥·그 밖 연도는 `dividend_disclosure` 로
  회사 하나씩 확인한다.
- **특별배당은 표시만 한다, 금액을 못 가른다.** `firm` 스코프의 「특별배당」 열은 비고에
  「특별배당」·「기념배당」이 실제로 박힌 결의만 켠다 — 다른 회사의 특별배당을 재원으로
  쓴다는 문장은 뺀다(코스피 FY2020~2024 전수 2/3,831 — 낮은 게 정상이다).
  정기·특별분이 한 결의에 합산돼도 서식에 분리 칸이 없어 이 표에서는 나눠 낼 수 없다.
  비고에 주당 특별배당분이 적힌 예외적인 경우만 [[dividend_disclosure]] 가 금액을 집어낸다.
