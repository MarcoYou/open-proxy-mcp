...
| valuation (scope=firm·explain) | ~15 | 11 | financial_metrics(5) + company + acntAll×3(CFS→OFS 폴백 시 +3) + stockTotqySttus + 배당. **KRX 시세=Supabase krx_weekly 서빙(serve-time 0콜)** · ECOS 0~1(캐시). scope=market/sector/firm_history = **DART·KRX 0콜(DB만)**. 실측 260705(API QA). 상세 [[valuation]] |
