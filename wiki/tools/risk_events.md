...
## 사용법
```
risk_events(company="예시기업 HG", include_details=True)        # 회사 24개월 전 카테고리
risk_events(company="예시기업 GE", category="embezzlement")     # 카테고리 필터
risk_events()                                                # 시장 전체 최근 30일 스캔
```

자연어 예시:
- "예시기업 HG 중대재해 공시 알려줘" / "최근 중대재해 발생한 기업들" (시장 스캔)
- "예시기업 GE 횡령 배임 공시 있어?" → 혐의발생→진행→사실확인 단계 추적
...
- **사상자 supersede 집계**: 같은 사건(발생일자+장소 정규화)의 원본·정정과 **지주사/사업회사 이중
  공시**(예시기업 AA+예시기업 Z, 예시지주 G+예시기업 C, 예시기업 HD+예시기업 HD에어로)는 최신 공시로 대체한다. 장소는 `㈜`/`(주)`·
