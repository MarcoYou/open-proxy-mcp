...
자연어 예시:
- "예시기업 CW 식별자랑 최근 공시 보여줘" → `query="예시기업 CW"`
- "예시기업 FW corp_code 확인" → `query="예시기업 FW"` (약칭 + special char 매칭)
- "예시기업 DA 배당" → `query="예시기업 DA"` (DART 공식 영문명 토큰 조합)
- "예시기업 J" → `query="예시기업 J"` (법인 접미사·구두점 정규화)
- "예시기업 CO" / "예시기업 CO" → 활성 상장 후보 중 시총 격차가 충분하면 예시기업 CW로 자동 추론하고 대안 표시
- "'예시기업 FP'라는 이름으로 상장사 여러 개면 뭐뭐 있어?" → 동명·유사명 후보 목록(status `ambiguous`)
...
{
  "company_id": "...",
  "canonical_name": "예시기업 CW(주)",
  "names": {"en": "예시기업 CW CO,.LTD", "aliases": [...]},
  "company_resolution": {
...
sequenceDiagram
    ...
    U->>T: query="예시기업 CW", max_recent_filings=10
...
- **강한 매칭이 약한 매칭을 이긴다** — 약한 tier(token/substring/fuzzy)에 먼저 걸려도 거기서
  멈추지 않고 역음차 경로를 끝까지 본다. 「예시음차 A」는 부분일치로 「예시기업 FK」에 걸려 「예시지주 H」를
  영영 못 찾고 있었다(「예시음차 B」→예시기업 EC도 같은 유형). 역음차가 약하게만 맞으면
  원래 결과를 유지한다 — 「예시음차 G」는 원문으로 아무것도 안 걸리고 'xyz' 토큰으로만 닿는다.
- **이름이 정확히 맞지 않으면 응답이 그 사실을 밝힌다** — 추정으로 고른 기업은 모든 tool 의
  `warnings` 맨 앞에 「「예시음차 A」를 **예시기업 FK**(으)로 추정했습니다」가 실린다. 적는 곳은 해석
...
- **법인격은 앞뒤 어디에나 붙는다** — DART 정식명은 「(주)예시기업 J5」·「주식회사예시기업 J6」처럼 앞에 오기도
  한다. `$` 앵커로 뒤만 떼면 공시에서 복사한 이름이나 **우리 tool 이 출력한 이름을 그대로 다시 물을 때**
  실패한다. 「예시기업 FJ」·「예시기업 EV」·「예시기업 CM」처럼 우연히 같은 글자로 시작하는 상호를 깎지
  않도록 닫는 괄호나 '식회사'를 요구한다.
- **역음차(reverse transliteration)** — DART 등록명이 라틴 표기인데 공고 헤더는 한글 음차로 적는다
  (예시음차 C=예시기업 AE · 예시음차 D=예시기업 B · 예시음차 E=예시기업 M). 알파벳 26자의 한글 음차표로 앞머리
  연쇄를 되돌린다. 「엔」은 알파벳 N이자 「엔터테인먼트」의 첫 글자라 어디까지 letter인지 정할 수 없어
  **길이별 변형을 모두 색인**하고(예시음차 G엔터테인먼트 → xyz엔터테인먼트), 질의·색인 **양쪽에 같은
  변환**을 걸어 반대 방향도 성립시킨다. 1글자는 우연 일치가 많아(예시기업 EZ의 '이'=E) 2글자부터다.
- **업종어 접미** — 공고 헤더는 정식 상호이고 등록명은 짧다(예시기업 CT→예시기업 CS · 예시기업 HU해상보험→
  예시기업 HU · 예시기업 BG공업→예시기업 BG). **짧게 떼는 것부터** 시도하고 후보가 정확히 하나일 때만 받는다 —
  「예시기업 CF보험」에서 '생명보험'을 떼면 「예시기업 CD」이라는 별개 회사가 나온다.
- **못 찾으면 근접 후보를 보여준다(자동 선택 X).** 앞자르기 자동선택은 오답을 낸다(예시음차 H→
  「예시기업 FU」·예시음차 F→「예시기업 AB」·예시지주 E→「예시지주 F」). 같은 계산을 '제안'으로 쓰면
  안전하다 — 고르는 것은 사람이다.
- NFKC·대소문자·공백·구두점·하이픈·법인 접미사를 정규화하고 `&`/`and`/`앤` connector를 같은 토큰으로 취급한다.
- 공식명 exact는 시총보다 절대 우선한다. `예시지주 A`는 예시기업 AL가 아니라 `예시지주 A`(034730)다.
...
공시 6건이 전부 「예시기업 V」로 나온다(당시 사명은 예시기업 EL정밀). 문서 본문의 「(구 예시기업 EL정밀)」 표기를
...
**한계**: 스냅샷을 남기기 시작한 뒤의 변경만 잡는다. 예시기업 EL정밀→예시기업 J7처럼 이미 지나간 변경은
복구할 수 없고, 그 경우는 「종목코드로 재조회」 안내로 물러난다.

## 변경 이력
- 2026-09-07: 최근 공시 표의 분류가 코드(`ownership_block`)로 나가던 것 → 우리말(대량보유 보고·주총 소집공고·배당 결정 …). 영어 응답은 코드 유지.
