...
- **큰 지역표를 통째로 놓치던 것(260802)** — 조정 행(부문간 제거)까지 실은 표에서 지역 정보를 못 찾아, **완전히 공시된 표를 버리고 있었다**(AJ네트웍스: 국내 1,113,506,004 / 해외 69,642,553 천원). 탐색 범위를 넓혀 해결. **전수 304건 회귀: 기존 95건의 값 변화 0 · 검출만 +3**(정형 1 · 원문폴백 2). 진단·수정 상세는 private lesson `production-call-signature-260802`.
- **「지역 정보가 다른 절에 있을 수 있다」는 안내가 대부분 헛짚고 있었다(260802)** — 미검출 사유 4분류 중 `outside_segment_note`(부문 주석 밖에 지역 표지가 있음)를 **문서 전체 단어 검색**으로 판정했다. 정기보고서는 수백만 자라 표지 어휘가 엉뚱한 맥락에서 나온다 — **6건 중 5건이 오탐**이었다: 위험관리 주석의 신용위험 익스포져 표(키움증권 「지역 합계」=금융자산 한국/기타) · 회사 주소(도이치모터스 「본사소재지 :」) · **없다는 선언**(신세계푸드 「지역별 정보는 산출하지 않습니다」 — 정반대 안내를 하고 있었다). 문맥으로 좁혔다(원문이 없다고 선언했으면 `not_disclosed` 로, 표지 주변에 매출·부문 문맥이 있고 주소·위험관리 문맥이 아닐 때만 유지). **전수 304건 회귀: 오탐 3건만 이동 · 다른 판정 변화 0**(extraction_failed 122 · no_segment_note 148 유지). 남은 3건은 롯데칠성음료 3개년으로 전부 진짜다 — 다만 표가 아니라 서술문이라 금액이 없다.
  - **안내 문구도 고쳤다(「위치 다름 · 다른 절에 실렸을 수 있습니다」 → 「표 없음 · 지역별 금액이 표로 공시되지 않았습니다」).** 탐색 창의 마지막이 **문서 전체**라 이 신호가 뜬 시점엔 이미 다 훑고 못 찾은 것이다(0부터 전수 재스캔해도 없음). 「다른 절을 찾아보라」는 없는 표를 찾으러 보내는 안내였다.
  - **서술 파서는 만들지 않았다.** 금액이 없어 매출 축에 넣을 수 없고(근거 없는 숫자 금지), 지역표 없는 208건 중 서술로 지역을 밝힌 건 4건인데 그중 1건이 이미 오탐(딜러 판매망 서술)이라 실질 회사 1곳이다. 오탐 있는 정규식을 그 규모에 넣을 값어치가 없다.
  - 절 단위로 제외하지 않은 이유: 롯데칠성의 진짜 지역 서술이 **「2. 중요한 회계정책」 절**에 있어, 절로 거르면 함께 죽는다.
- **귀속기준을 원문에서 가져오거나 없다고 밝힌다(260802)** — 출처 라벨의 「고객 소재지」를 뺀 뒤(위 항목), 실제 귀속기준을 원문에서 뽑아 `attribution_basis` 로 싣는다. **실측 96건 중 5건(5%)만 공시**하고, 그나마 「고객 소재지」만 있는 게 아니다 — 「지역별 외부고객으로부터의 수익(**사업장 소재지** 기준)」도 있다. 못박았으면 나머지 95%에서 거짓이었다. 없으면 「귀속기준 미공시 — 어느 나라 매출로 잡았는지는 회사가 밝히지 않았습니다」로 밝힌다.
  - 원문에서 귀속기준 **문장**을 찾아 그대로 싣는다(추출 방식은 private lesson).
...
- **지역별 표가 연결인지 별도인지 밝힌다(260802)** — 출처 라벨에 「**연결** 기준 · 고객 소재지」가 하드코딩돼 있어, **별도 절을 읽고도 연결이라 말했다**(실측 95건 중 5건: 대웅제약 2개년·코오롱글로벌·한글과컴퓨터·디에이치오토웨어). 귀속기준도 마찬가지로 실제 공시는 5.8%뿐이고 57건은 「본사 소재지 국가」(법인 소재지)라 라벨이 사실과 달랐다. 라벨에서 둘 다 빼고, 축 노드가 실제 값을 싣는다 — `basis`(XBRL 코드 `NT_C`/`NT_S`로 판별, 코드 없으면 「미상」) + `basis_conflict`(연결 절이 있는데 별도를 읽었을 때만).
  - 연결 절을 먼저 보게 하는 것만으로는 안 고쳐진다(원인 분석은 private lesson). 값을 버리면 후퇴이므로 **값은 두고 무엇을 읽었는지 밝히는** 쪽을 택했다.
