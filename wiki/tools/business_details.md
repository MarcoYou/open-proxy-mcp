...
- **한계**: ① 필드 제목 자체가 없고 일반 본문에만 값이 있는 경우는 정밀도 우선으로 `NOT_COLLECTED` 가능 ② 금융지주·REIT는 segments `UNSUPPORTED_FORM`(D-트랙 별도) ③ customers 고객명 다수 익명(주요고객A/B) — 이름 억지생성 안 함 ④ 실제 연구개발 상세표가 수만 자인 회사는 응답도 큼(특정 `fields` 지정 권장) — **금융지주·보험은 특히 크다**(260728 실측: KB금융지주 145,181자 = 재무건전성 70,710 + 영업의 현황 44,769. 이 두 필드는 상한이 없다. `revenue_mix_form`·`key_contracts`만 20,000자 상한 적용) ⑤ `key_contracts`는 기중 종료된 계약을 기말 기준 `NOT_APPLICABLE`로 낸다(세방전지: Dr. Wagner 기술도입계약 2025-06-30 종료 → 해당없음). 사실은 맞으나 정보가 접힌다 — 286건 중 1건.
