...
> "예시기업 EG 생산능력이랑 가동률 어떻게 돼?"
>
> "예시기업 H 수주잔고 얼마나 쌓여 있어?"
>
> "예시기업 CW 사업부문별 매출이랑 영업이익 나눠서 보여줘"
...
- 예: `business_details("예시기업 EG", fields="utilization")` · `business_details("예시기업 H", fields="backlog")` · `business_details("예시기업 CW", bsns_year="2025", reprt_code="11014")`(2025 3분기 스냅샷)
...
|---|---|---|---|
| **segments** | 영업부문 주석(K-IFRS 1108) | 부문별 매출·영업이익 (정형표 or 원문 마크다운) | 예시기업 CW(정형)·예시기업 AL(후보)·예시기업 J3(마크다운) |
| **sites** 사업장 | 3.원재료 및 생산설비 / 사업장 현황 | 공장·사업장 소재지·면적 원문(유통은 점포) | 예시기업 CW(공장 주소)·예시기업 EY(점포 창동점) |
| **utilization** 가동률 | 생산실적 및 가동률 | 생산능력·생산실적·가동률 원문(+% 힌트) | 예시기업 AL(100%)·예시기업 EG(생산능력 톤) |
| **rnd** 연구개발 | 연구개발활동 | 연구개발비·실적·조직 원문(+매출대비% 힌트) | 예시기업 EV·예시기업 GX |
| **backlog** 수주 | 4.매출 및 수주상황 | 수주잔고·수주계약 원문(조선 flow표 포함) | 예시기업 H(기초계약잔액→수주잔고) |
| **customers** 고객 | 주요 매출처 / 주요 고객 주석 | 주요 고객·매출처·판매경로 원문(익명 다수) | 예시기업 CW·예시기업 EV |
| **raw_materials** 원재료 | 주요 원재료 현황 / 원재료 가격변동추이 | 원재료 구성·매입과 원재료 가격 추이 원문 | 예시기업 Y·예시기업 BJ |
| **product_pricing** 제품가격 | 주요 제품 등의 가격변동추이 | 판매가격·ASP·가격변동 원인 원문 | 예시기업 CW·예시기업 H |
| **revenue_breakdown** 매출 분해 | **매출 축 4개의 단일 진입점** | `by_segment`(III 주석·감사O·**매출+이익**) + `by_product`(II-2-가·감사X·매출만) + `by_region`(III ¶33·**연결**·매출만) + `by_trade`(II-4 매출실적표·**별도**·수출/내수) + `available`/`needs_review` | 예시기업 HM(4축 전부)·예시기업 J(부문 없음→제품 있음) |
| ~~geo_revenue~~ → `by_region` | 위와 동일 | **옛 이름은 별칭으로 유지** — `fields="geo_revenue"` 로 부르면 종전대로 평평하게 반환(옛 호출 비깨짐). 새 코드는 축 이름을 쓴다 | 예시기업 AV·예시전지회사 A(단일부문사) |
| **revenue_mix_form** 매출구성 | II-2-가 주요 제품 등의 현황 | 제품·품목별 매출액·비중 원문 + `self_check` | 예시기업 J(단일부문인데 제품 3종)·예시기업 DP |
| **key_contracts** 주요계약 | II-6-가 주요계약 | 라이선스·기술도입·장기공급 계약 원문 | 예시기업 BB(라이선스아웃/인)·예시기업 BF |
| **financial_ops** (금융) | 2.영업의 현황 | 영업개황·영업실적·**영업부문별 재무정보**(금융판 segments) | 예시기업 DQ·예시기업 CH |
| **financial_soundness** (금융) | 재무건전성·지급여력 | RBC·지급여력비율(K-ICS)·순자본비율·연체율 | 예시기업 CS·예시기업 ES |
| **investment_property** (REIT/보험) | 투자부동산 내역·투자자산 개요 | 부동산 목록·임대율·임대면적·공실 | 예시기업 AG·예시기업 CS |
...
> **KSIC(업종코드) 게이트** — content 마커만이면 예시기업 FP(포털)·예시기업 HD(화약)·예시기업 DT(화장품)이 자회사/우발 신호로 오발. → `get_company_info`의 `induty_code`로 **금융권(KSIC 64/65/66)일 때만** financial 필드, **부동산(68)/보험(65)/지주(64)**일 때만 investment_property. 비금융은 아예 시도 안 함(오발 0). **지주회사 64992는 충돌**(예시금융지주 A vs 예시지주 A)이라 그 안에서만 content-signature로 판별. 응답에 `induty_code` 동봉.
...
> **D-트랙은 II.사업의내용 구간만 스캔**(`_biz_html_region`) — 시그니처 폴백이 full html을 훑으면 **III.재무 주석의 회계표**(공정가치 서열체계·투자부동산 장부금액 헤더조각)를 REIT 투자부동산으로 **오발**(NH올원·예시기업 EX: 1095·398자 회계각주). II→III 경계는 **목차 stub(수십자) skip 후 첫 실질구간(>2000자)**으로 잡는다(max-span 금물 — 예시기업 HE류는 말미에 종속사 사업보고서가 embedded돼 그 span이 본문보다 커 오선택→본문 DP Real Estate·종속REIT 서술 통째 누락). II 제한 후 NH올원·이리츠는 오히려 II 프로즈(reit_prose 7203·2123자)로 정상 회수, 예시기업 HE은 종속 부동산 서술 유지, 은행·증권 financial 필드·정상 REIT 전부 불변.
> **지주형 REIT 프로즈 폴백**(`source=reit_prose`) — 명목회사(해외리츠 등)는 표준(제조)폼에 부동산을 **서술형**으로 싣는다(예시기업 FG: 파이낸스타워(벨기에)·498 Seventh Avenue를 '2.주요 제품 및 서비스→영업개황'에 임대료·WALE·임차율 프로즈로. 전용 투자부동산 헤딩·표 없음). 전용 헤딩/시그니처가 다 실패할 때만 표준폼 헤딩(주요 제품 및 서비스·영업개황·회사의 현황)을 시도하되 **content-gate 강화**(임대료/임대차/임차 + 부동산/투자대상/WALE 동반)로 보험 영업개황 등 오섹션을 차단. 작동하는 REIT는 전용경로에서 이미 반환돼 영향 없음(예시리츠 A·예시지주 A·예시지주 D·예시리츠 B 전부 `source=heading` 유지 확인).
...
- `geo_revenue`(지역별 수익 — 전사 차원 공시):
  ...
  - **`foreign_share_pct` 해외 매출 비중 — 이 필드를 절대금액보다 앞세운다.**
    ...
    지역명에 각주가 붙으면(예시기업 FP 「국내(주1)」) 국내로 못 읽어 100%가 나오므로 각주를 뗀다.
    표에 국내 구분 항목이 아예 없으면(예시기업 BK) `share_caveat`로 밝힌다 — 「해외 100%」와
    「국내 항목이 표에 없음」은 다르다.
  ...
  - `SUCCESS_NO_TOTAL_COLUMN` → 합계 열이 없지만 항목이 **전부 지역명**이라 항목합을 총계로
    쓴 경우(예시기업 J 「외국 | 본사 소재지 국가」 2칸). 검산은 못 했다고 밝힌다.
...
- `revenue_breakdown`(매출 분해 — **기본 반환 세트의 단일 진입점**):
  - **묻는 곳을 하나로 합친 필드다.** `segments`만 물으면 「단일부문 선언」을 보고 "이 회사는 부문
    정보가 없다"로 끝나는데, 실제로는 II-2-가에 제품별 구성이 있다(예시기업 J 전력기기 69.5%).
  ...
  - `available`은 값이 나온 축, `needs_review`는 **원문만 있고 값은 못 믿는 축**. 섞지 않는다
    (예시기업 AZ: 제품별이 시공실적 표라 검토필요인데 available 에 넣으면 안내가 거짓이 된다).
...
- `revenue_mix_form`(매출구성 — 기업공시서식 II-2-가):
  - **`segments`와 다른 자료다.** `segments`는 III 주석의 K-IFRS 1108 영업부문(외부감사 대상),
    ...
    여기 있다** — 예시기업 J은 부문 주석이 「단일부문」인데 II-2-가에는 전력기기 69.5% ·
...
- **귀속기준은 원문에 있을 때만 싣는다**(`attribution_basis`). 실제로 밝히는 회사는 소수이고,
  ...
  이 값이 **해외비중의 의미를 좌우한다**: 예시기업 BJ 국제선은 「본사 소재지 국가」로 잡혀 해외비중이
...
검증: 120사 전수 before/after 대조 **289만 자, 차이 0** — 유일한 양성 케이스(예시기업 O
