...
- **유통·서비스·건설 사업장 헤딩 확대(260718 5산업군 스캔)** — 업종별로 사업장 소절명이 다르다: 편의점 "영업설비"(BGF리테일 점포현황), 호텔·면세 "영업장 현황"(호텔신라), 건설 "주요 설비의 현황"(대우건설·DL이앤씨 토지/건물 소재지). `_SITE_HEAD`에 `영업\s*설비`·`영업장\s*현황`·`주요\s*설비의?\s*현황` 추가 — content-gate `_C_SITE`(소재지·점포·㎡ 등)로 6사 N/A→md(카카오·HLB 포함), sites-only 변화·타필드 불변. 지주/유통 util=md는 오발 아님(종속 제조/통신사 실가동률: 현대백화점=지누스, GS리테일=후레쉬서브, SK=SK온, LG=LG유플러스). **미해결**: "(제조서비스업)/(금융업)" 업종 프리픽스 헤딩(카카오·HLB 복합기업)은 `_SUBSEC_PREFIX`가 못 뚫음 — 별건.
