...
회사(한국전력·예시기업 AW·예시기업 AP)가 있어 한 회사의 승인 한도가 여러 행에 흩어진다 — 한 행만
...
**`sm`은 그 행의 인원 합계**다 (예시기업 HM: 남 정규직 58,225 + 계약직 8,456 = 66,681 = `sm`).
정규직/계약직 개별 필드는 원문 자체에 오타가 실리기도 하지만(예시기업 GD: `rgllbr_co`가 981로 오기재)
`sm`은 DART가 따로 계산해 넣어 다른 행들과 내부 정합이 유지된다. 단 `sm` 자체가 없는 회사도 있다.

**부문별 응답 구조는 2갈래다.** 대부분은 부문·성별 상세행에 급여가 채워진다(예시기업 HM 남/여).
예시기업 CW류(부문별+성별합계 양식)는 부문 상세행(DX/DS 등)이 공백("-")이고 **'성별합계' 행에만
