...
> `director_board` tool이 쓰는 6개 정형 API의 **공식 필드명 사전**. 260708 세션에서 값 패턴만 보고
> `sm` 필드 의미를 추론했다가("합계일 것") 사용자가 "정확한 필드명 가져와서 확인해야 하는 것
> 아니냐"고 지적 — DART 개발가이드(opendart.fss.or.kr/guide)에서 공식 매핑을 가져와 확정했다.
> **값 패턴 역추론보다 공식 문서 우선**이 원칙 — 이 페이지가 그 인덱스.
...
**주의(260708 실측)**: `rgist_exctv_at`(등기 임원 여부)의 실제 값은 "등기"/"미등기"가 아니라
**이사 구분**(사내이사·사외이사·기타비상무이사·감사)으로 나온다 — 필드명과 실제 값 의미가
다르니 코드에서 "등기 여부" 문자열 매칭을 하면 안 됨(director_board.py에서 이 값 집합을
`_BOARD_TYPES`로 직접 열거해 대응).
...
**주의(260708 버그 이력)**: `se`(구분)가 회사마다 자유 텍스트라 상임/비상임·등기/사외로 **분리
공시**될 수 있음(한국전력·기업은행·강원랜드) — 여러 행을 무조건 합산해야 하며, 마지막 행으로
덮어쓰면 한도가 축소되는 버그가 있었다(수정 완료, `director_board.md` 참조). "감사위원회 위원
또는 감사" 같은 결합 표기도 있어 감사 전용 행 판정은 `se`에 "이사" 문자열이 아예 없는지로 해야
함("위원" 유무로 가르면 오분류).
...
`psn1_avrg_pymntamt`(1인평균)는 API가 이미 계산해 제공 — `pymnt_totamt÷nmpr`와 정합 확인됨
(260708 IR 검증). 스톡옵션 관련 필드(`stk_opt_*`)는 director_board 소진율 계산 이상치(실리콘투·
휴젤 등) 조사 시 "행사이익 혼입" 가설 검증에 썼으나 13개사 전부 공백으로 기각됨.
...
**`sm`("합계") 확정 근거(260708)**: 현대차 실측 — 남 정규직 58,225 + 계약직 8,456 = 66,681 = `sm`.
`director_board.py`의 `_emp_row_usable`가 인원수를 `sm` 1순위로 쓰는 이유 — 정규직/계약직 개별
필드에 원문 자체 오타가 있어도(클로봇 사례, `rgllbr_co`가 981로 오기재) DART가 별도 계산해 제공하는
`sm`은 다른 행들과 내부 정합이 유지돼 더 신뢰할 수 있음(단, `sm` 자체가 없는 회사는 정규+계약
합산으로 폴백).
