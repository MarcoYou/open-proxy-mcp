...
> 한국 상장사 거버넌스 관련 강행규정 + 우회 catalog. 의결권 권고 엔진의 법령 layer 출처.
...
의결권 권고 엔진은 강행규정 적용 여부를 **오늘이 아니라 '이 주총'의 주총일** 기준으로 판단한다
...
의결권 권고 엔진이 띄우는 근거(`law_reference`)는 **정확한 조문번호 + 시행령 임계 출처**를 담는다.
...
- **정책 — 참조 조문은 개정대장에 안 넣는다**: `law_provisions.json`은 **상법 2025–2026 *개정* 대장**이다.
  ...
  개정, 의결권 권고 엔진의 장기연임 룰이 인용)은 개정대장에 넣으면 3자 정합[7]을 오염시키므로 **넣지 않고**, verifier의
  `REFERENCE_PROVISIONS`로 원문 존재만 확인한다. 코드는 그 조문을 정밀 인용(§34조5항7호, 원문 대조 확정).

## 옛 분산 자료

종합본에 흡수된 옛 페이지(상법개정-통합본·타임라인·정관-우회-시나리오·주총방어-시나리오·주총체크리스트)는
현재 레포에 없다 — git 이력에만 남아 있다(`wiki/archive/` 폴더는 폐지, 260902).
...
3. **출처 reference 자료**: `wiki/raw/` 에 원본을 두고 본 README 에서 link

→ 신규 분산 X. 항상 master 1개.
