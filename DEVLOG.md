...
- `main` 기준으로 `open-proxy-mcp-v1.3.0` 브랜치 생성 (v1 보존)
- 불필요한 브랜치 정리: `release_v2.0.0`, `release_v2`, `flyio-new-files`, `cursor/*`, `refactor/architecture`, `feat/*`
- GitHub 태그 `v2.0.0` (main), `v1.3.0` (이미 존재) 생성
- GitHub Releases 생성 (v2.0.0)

### fly.io 배포 + CI/CD

- `fly deploy` 수동 배포 완료 (nrt, 2 machines)
- `fly tokens create deploy`로 CI 전용 deploy 토큰 생성
- GitHub Secret `FLY_API_TOKEN` 등록 → GitHub Actions CI/CD 정상화
- `OPEN_PROXY_TOOLSET=v2` 확인, tool 11개 정상 노출

### 문서 정비

- README.md 전면 개편: v1 스타일 유지 + v2 구조 반영, 비교 사진 복원, 로컬 설치 섹션 제거
- README_ENG.md 동일 구조로 전면 개편
- docs/ARCHITECTURE.md v2 기준 전면 업데이트 (tool/service 분리, data flow, cache, rate limiting)

## 2026-04-18

### shareholder_meeting v2 2차 확장
- `scope=board|compensation|results` 추가
- `meeting_type=auto` 기본화
  - 정기 최신 회차 vs 임시 최신 회차 후보 생성
  - 일반 조회는 정기/임시를 가리지 않고 가장 현재적인 회차 우선
  - `results` 조회는 결과공시 확인 최신 회차 우선
  - `selection_basis`, `selected_meeting`, `alternative_meetings` 출력
- 최근 12개월 커버리지 추가
  - 조사 대상은 전체 공시가 아니라 `주주총회소집공고` 제목군만
  - `annual_only | extraordinary_only | annual_and_extraordinary | none`
  - selected meeting 기준 12개월 window로 계산
  - summary에서 최근 정기/임시 회차 메타까지 표시
  - `auto` 후보도 최근 12개월 기준 `가장 최근 정기 1개 + 가장 최근 임시 1개`
  - 교차연도 임시주총은 그 회차의 실제 회의연도로 결과공시 검색
- `meeting_phase`, `result_status` 도입
  - `pre_meeting`
  - `post_meeting_pre_result`
  - `post_result`
  - `undetermined`
- 결과는 무조건 쓰지 않고:
  - DART `주주총회결과` 탐색
  - whitelist 가능한 `rcept_no(80)`만 KIND `acptno(00)`로 변환
  - 실패 시 `requires_review`
- summary에도 이제 “결과가 아직 안 나온 주총”과 “결과까지 확인된 주총”을 구분 표시

### shareholder_meeting v2 실조회
- `KT&G`, `auto`, `2026`, `summary`
  - `meeting_type=annual`
  - `meeting_phase=post_result`
  - `result_status=available`
  - 결과 공시 `20260326802654`
- `KT&G`, `annual`, `2026`, `board`
  - 후보 3명 확인
- `KT&G`, `annual`, `2026`, `compensation`
  - 당기 보수한도 `6,000백만원`
  - 전기 실제 지급 `2,445백만원`
- `KT&G`, `auto`, `2026`, `results`
  - KIND `20260326002654`
  - 안건별 찬성률/반대율 파싱 성공
- `한화`, `auto`, `2025`, `summary`
  - 더 최근인 임시주총 자동 선택
  - 대안으로 정기주총 같이 표시
- `아시아나항공`, `auto`, `2025`, `summary`
  - 더 최근인 정기주총 자동 선택
  - 대안으로 임시주총 같이 표시
- `KT&G`, `auto`, `2026`, `summary`
  - coverage `annual_only`
- `한화`, `auto`, `2025`, `summary`
  - coverage `annual_and_extraordinary`

### ownership_structure control_map 고도화
- raw 표 중심에서 `control_map` 해석형 구조로 변경
- 핵심 블록
  - `core_holder_block`
  - `treasury_block`
  - `overlap_blocks`
  ...
  - `flags`
  - `observations`
- 이름 정규화로 최대주주 명부와 5% 블록의 겹침 여부 표시
- 관찰 포인트는 5% 이상 능동 블록 기준으로만 생성

### ownership_structure 실조회
- `삼성전자`, `control_map`
  - `삼성물산`을 명부와 겹치는 능동 블록으로 표시
- `고려아연`, `control_map`
  - `한국기업투자홀딩스`, `최윤범`, `크루시블제이브이`를 겹치지 않는 능동 블록으로 표시
- `한화`, `control_map`
  - 특수관계인 50% 이상
  - 자사주 5% 이상
...
### release_v2.0.0 코드 착수
- `open_proxy_mcp/tools_v2/` 신규: v2 public facade 전용 레이어 시작
- `open_proxy_mcp/services/` 신규: v2 공통 business logic/service 레이어 시작
- `server.py` 리팩토링: import 시점 고정 등록 대신 `build_mcp(toolset)` 구조로 변경
- toolset 선택 지원 추가
  - `--toolset v1|v2|hybrid`
  - `OPEN_PROXY_TOOLSET` 환경변수 연동
- `__main__.py` 단순화: `main()` 직접 호출 구조로 변경

### v2 공통 계약 추가
- `services/contracts.py` 신규
- `AnalysisStatus`: `exact / ambiguous / partial / conflict / requires_review / error`
- `SourceType`: `dart_api / dart_xml / dart_html / kind_html / naver / internal`
- `ToolEnvelope`, `EvidenceRef`, pretty JSON helper 추가
- 목적: 앞으로 `shareholder_meeting`, `ownership_structure`, `dividend` 등도 같은 응답 껍데기를 쓰게 하기 위함

### company v2 facade 첫 구현
- `tools_v2/company.py` 신규: analyst-facing public tool `company`
- `services/company.py` 신규: 실제 회사 식별 + 최근 공시 인덱스 조립 로직
- 기존 `corp_identifier`와 다른 점
  - partial match를 자동 확정하지 않음
  - exact가 아니면 `ambiguous`로 멈춤
  - 회사 카드 + 최근 공시 인덱스를 한 번에 반환
- 현재 반환 항목
  - `company_id`
  - `ticker / corp_code / jurir_no / bizr_no`
  - 시장 / 업종 / 결산월
  - 대표이사 / 홈페이지 / 주소
  - 최근 공시 인덱스 (`report_name`, `rcept_no`, `filing_type`)
- 최근 공시는 DART `list.json` 기준, 업종은 NAVER 보강
- ISIN은 아직 미연결 상태로 경고만 표시

### 검증
- `python -m compileall open_proxy_mcp` 통과
- `build_mcp('v2')` import 확인
- 샘플 실조회
  - `build_company_payload('삼성전자', max_recent_filings=3)`
  - 결과: `exact`, `cmp_005930`, 최근 공시 3건 반환

### 다음 단계
- `shareholder_meeting` v2 facade 골격 추가
- `company`에서 recent filing을 v2 탭 이름과 더 가깝게 정규화
- `evidence` 스키마를 public tool 수준까지 확장

### shareholder_meeting v2 1차 구현
- `services/shareholder_meeting.py` 신규
- `tools_v2/shareholder_meeting.py` 신규
- 현재 scope 지원
  - `summary`
  - `agenda`
- 현재 동작 원칙
  - `company`와 동일하게 회사 식별 exact 아니면 자동 선택 금지
  - `annual|extraordinary`를 한 tool에서 처리
  - 소스는 `DART list.json + DART XML`
  - PDF fallback 없음
  - 안건 파싱 신뢰도 낮으면 `requires_review`

### shareholder_meeting 현재 반환 구조
- `company_id`
- `notice` (공시명 / 공시일 / rcept_no / 정정 여부)
- `meeting_info` (정기/임시, 기수, 일시, 장소, 보고사항)
- `agenda_summary` (루트 안건 수 / 전체 안건 수 / 상위 안건 제목)
- `agendas` (`scope=agenda`일 때)
- `correction_summary` (정정공고인 경우)
- `evidence_refs` (DART XML notice 근거)

### shareholder_meeting 검증
- `KT&G` alias 입력 검증
  - 입력: `KT&G`
  - 식별: `케이티앤지`
  - `company_id`: `cmp_033780`
- `2026 annual summary`
  - `rcept_no`: `20260225005779`
  - `meeting_type`: `정기`
  - `agenda_total_count`: `15`
- `2026 annual agenda`
  - 루트 안건 수: `8`
  - 첫 안건: `제1호 제39기 재무제표 및 이익잉여금처분계산서 승인의 건`

### 다음 단계 (업데이트)
- `shareholder_meeting`에 `board`, `compensation`, `results` 순서로 scope 추가
- `results`는 KIND whitelist 정책과 연결
- `company` recent filing 분류를 v2 탭 구조와 더 가깝게 정리

### 남은 v2 data tool 일괄 구현
- 신규 서비스
  - `services/ownership_structure.py`
  - `services/dividend_v2.py`
  - `services/value_up_v2.py`
  - `services/proxy_contest.py`
  - `services/evidence.py`
- 신규 public facade
  - `tools_v2/ownership_structure.py`
  - `tools_v2/dividend.py`
  - `tools_v2/value_up.py`
  - `tools_v2/proxy_contest.py`
  - `tools_v2/evidence.py`

### ownership_structure v2
- 지원 scope
  - `summary`
  - `major_holders`
  - `blocks`
  ...
  - `timeline`
- 소스 정책
  - 최대주주/주식총수/자사주: DART 공식 API
  - 5% 보유목적: API 우선, 필요 시 DART 원문 보강
- 반환 요약
  - 최대주주
  - 특수관계인 합계
  - 자사주 수/비중
...
- 지원 scope
  ...
  - `signals`
  - `timeline`
- `vote_math`는 아직 비공개 (`requires_review`)
- 소스 정책
  - 위임장/공개매수: DART D
  - 소송/분쟁: DART I/B
  - 5% 시그널: DART majorstock + 원문 목적 보강

### evidence v2
- `evidence_id` 또는 `rcept_no`로 원문 발췌 조회
- 현재는 DART XML 기반 evidence를 우선 지원
- keyword가 있으면 해당 주변 snippet 우선 반환

### 남은 v2 data tool 검증
- `python -m compileall open_proxy_mcp` 재통과
- `build_mcp('v2')` 성공
- 샘플 실조회
  ...
  - `proxy_contest('고려아연', summary, 2026)` → `exact`, `cmp_010130`
    - 위임장/공개매수 `7건`
    - 주주측 문서 `4건`
    - 소송/분쟁 `40건`
    - 능동적 5% 시그널 `4건`
  - `evidence(rcept_no='20260225005779', keyword='제39기')` → `exact`

### 다음 단계 (재정리)
- `shareholder_meeting`에 `board`, `compensation`, `results` 추가
- `results`에 KIND whitelist 정책 연결
- `ownership_structure` control map 정규화 고도화
- `proxy_contest`의 vote_math는 후속 단계로 유지

## 2026-04-18 | ownership_structure control_map 고도화
- `control_map`를 단순 raw dump가 아니라 판 구조를 읽는 화면으로 재구성
- 핵심 블록:
  - `core_holder_block`
  - `treasury_block`
  - `overlap_blocks`
  ...
  - `observations`
- 해석 기준:
  - 최대주주 명부와 5% 블록 이름이 겹치면 `registry_overlap`
  - 의미 있는 능동 블록은 5% 이상 위주로 관찰
- 실조회 메모:
  - `삼성전자`: `삼성물산`은 명부 겹침 능동 블록
  - `고려아연`: `최윤범`, `크루시블제이브이`, `한국기업투자홀딩스`는 비겹침 능동 블록
  - `한화`: 명부상 특수관계인 50%+, 자사주 5%+

## 2026-04-18 | proxy_contest 재정렬
- 최근 12개월 기준으로 분쟁 공시를 보는 구조로 정리
- 위임장/공개매수, 소송/분쟁은 제목군 타깃 검색
- `ownership_structure.control_map`과 연결해 플레이어를 같이 읽게 변경
- 새 요약 축:
  - 회사측 제출인
  - 주주측 제출인
  - 명부와 안 겹치는 능동 5% 블록
  - 명부와 겹치는 능동 5% 블록
- `fight`에는 `actor_group`, `signals`에는 `actor_side`, `timeline`에는 `actor/side` 추가
- 최근 12개월 밖 5% 시그널은 제외
- 실조회 메모:
  - `고려아연`: 회사측 `고려아연`, 주주측 `영풍`, 외부 능동 블록 `최윤범/크루시블제이브이/한국기업투자홀딩스`
  - `한화`: 최근 12개월 시그널은 `김승연` 1건으로 정리

## 2026-04-18 | 날짜 파라미터 표준화
- v2 public data tool에 시간축 추가
  - `company`: `start_date`, `end_date`
  - `shareholder_meeting`: `start_date`, `end_date`, `lookback_months`
  - `ownership_structure`: `as_of_date`, `start_date`, `end_date`
  - `dividend`: `start_date`, `end_date`
  - `proxy_contest`: `start_date`, `end_date`, `lookback_months`
  - `value_up`: `start_date`, `end_date`
  - `evidence`: `start_date`, `end_date` 시그니처 통일
- 공통 유틸 추가: `services/date_utils.py`
- ownership은 `as_of_date`를 직전 사업연도 스냅샷으로 연결
- evidence는 아직 `rcept_no` 직접 조회가 우선이라 window는 메타데이터로만 보관
- 검증 메모:
  - `company('삼성전자', 2026-03-01~2026-04-18)` 정상
  - `shareholder_meeting('한화', 2025-12-01~2026-04-18)` 정상
  - `ownership_structure('한화', as_of=2026-04-18, 2026-01-01~2026-04-18)` timeline 4건
  - `dividend('삼성전자', 2024-01-01~2025-12-31)` 최근 결정 5건
  - `proxy_contest('고려아연', 2025-12-01~2026-04-18)` 능동 시그널 4건
  - `value_up('KB금융', 2025-01-01~2026-04-18)` timeline 3건

## 2026-04-18 | prepare_vote_brief 1차
- 첫 action tool 추가
- 목표:
  - 회차 선택
  - 판 구조
...
- 아직 안 하는 것:
  ...
  - engagement 문안 자동 생성
- 구현 메모:
  - `shareholder_meeting`의 summary/agenda/board/compensation/results 사용
  - `ownership_structure control_map` 사용
  - `meeting_date`를 `ownership as_of_date`로 넘겨 같은 회차 기준으로 맞춤
  - evidence는 하위 tool 근거를 합쳐서 dedupe
- 검증 메모:
  - `KT&G`, `2026` 정상
  - `한화`, `2025-12-01~2026-04-18` 정상
  - 한화에서 반대율 10%+ 안건 2개 포착

## 2026-04-18 | engagement/campaign action tool 추가
- `prepare_engagement_case`
  - `ownership_structure + proxy_contest + value_up`를 묶는 engagement memo
  - 처방이 아니라 fact-first engagement case
- `build_campaign_brief`
  - `shareholder_meeting + ownership_structure + proxy_contest`를 묶는 campaign fact brief
  - vote math나 추천 없이 플레이어/지배구조/타임라인 중심
- 검증 메모:
  - `prepare_engagement_case('KT&G', 2025-12-01~2026-04-18)` 정상
  - `build_campaign_brief('KT&G', 2026)` 정상

## 2026-04-18 | proxy_contest vote_math 추가
- `proxy_contest(scope="vote_math")` 공개
- 로직:
  - `shareholder_meeting(results)`에서 KIND 결과표를 받아옴
  - 안건별 추정참석률 = 발행기준 찬성률 / 행사기준 찬성률
  - 보통결의 안건 최빈값을 대표 추정참석률로 사용
  - 감사위원/집중투표/비교 불가 안건은 제외
- 출력:
  - 대표 추정참석률
  - 특수관계인/자사주/능동 블록 관점의 표 구조
  - 특수관계인 제외 추정 참석분
  - 고반대율/부결 안건
  - signal_level (`stable` / `watch` / `contestable`)
- 가드레일:
  - 승패 예측 엔진으로 보이지 않게 fact-first
  - 결과공시 없거나 비교 가능한 보통결의 안건 부족 시 `requires_review`

## 2026-04-18 | 요약형 KIND 결과공시 파서 추가
- 배경:
  - 2025 이전/일부 회사 결과공시는 `주주총회 안건 세부내역` 표 없이 `결의사항 -> 원안가결/부결`만 제공
  - 예: 한화 2025 임시주총, 한국전력 2025 정기주총
- 구현:
  - `shareholder_meeting(results)`에서 표형 파서 실패 시 요약형 파서 fallback
  - `제1호 의안`, `제2-1호 의안` 같은 번호와 가결/부결 추출
- 출력:
  - `result_format=summary`
  - `numerical_vote_table_available=False`
  - 안건별 결과는 보이되 찬성률/참석률은 비움
- 원칙:
  - 요약형은 결과 확인용
  - `vote_math`는 계속 표형 결과공시에만 의존

## 2026-04-18 | prepare_vote_brief에 결과 품질 / vote_math 연결
- `prepare_vote_brief`가 결과공시 품질을 같이 보여주도록 보강
- 세부표형이면:
  - `result_format=table`
  - 수치표 제공 여부 표시
  - `vote_math` 요약 연결
...
## 2026-04-18 | prepare_vote_brief에 집중투표 사전 전략 블록 추가
- `cumulative_voting_strategy` 추가
- 계산:
  - 집중투표 대상 이사 수
  - 자사주 차감 후 의결권 모수
  - 100% 참석 가정 1석선
  ...
  - 최대주주/능동 블록과의 격차
- 가드레일:
  - 공시에 집중투표가 명시되지 않으면 `partial`
  - 감사위원/분리선출은 보수적으로 제외

## 2026-04-18 | shareholder_meeting DART viewer crawl fallback
- `document.xml` 파싱 품질이 낮으면 DART `main.do -> viewer.do` HTML crawl로 재시도
- `DartClient.get_viewer_document()` 추가
- `shareholder_meeting` summary/agenda/board/compensation 경로에 fallback 판단 로직 추가
- `notice_parse_source=dart_xml|dart_html` 메타 추가

## 2026-04-18 | KT&G 2024 구형 요약형 결과공시 보강
- 샘플: KT&G 2024 정기주총 결과 `20240328801345`
- `- 제1호 : ...`, `☞ 제2-1호 내지 제2-6호 승인`, `☞ 제3-1호 및 제3-3호 가결, 제3-2호 부결` 패턴 대응
- 결과:
  - `shareholder_meeting(results)`가 12개 안건 결과를 `summary` 형식으로 구조화
  - `vote_math`는 숫자표 부재로 `requires_review / numerical unavailable` 유지

## 2026-04-19 | action tool source quality 메타 전파
- `prepare_vote_brief`, `prepare_engagement_case`, `build_campaign_brief`에 quality 블록 추가
- `notice_parse_source`, `result_format`, `numerical_vote_table_available`, component status 표시
- 2026-04-19
  - v1 대비 v2 실패 원인 구분 보강
  - `value_up_v2`: `availability_status`, `search_diagnostics` 추가
  - `dividend_v2`: `history_selection` 추가, 최근 완료 사업연도 기준 3개년 선택으로 수정
  - 확인:
    - 현대자동차 밸류업: 요청 구간/진단 구간 모두 공시 없음
    - 삼성전자 배당 히스토리: 2023/2024/2025 3개년 복구

## 2026-04-19 | 제목 타깃 검색 공통화 + DART/KIND taxonomy 반영
- 문제 정리:
  - `현대자동차 기업가치제고계획`은 DART에도 있었지만, 긴 기간 조회에서 첫 페이지만 보고 놓친 케이스였음
  - 원인은 `공시 없음`이 아니라 `pagination + 제목 타깃 검색 부족`
- 조치:
  - `services/filing_search.py` 추가
  - 공통 원칙을 `회사 + 기간 + pblntf_ty + 공시명(report_nm) 타깃 검색`으로 통일
  - 무한 pagination은 하지 않고, 지정 기간 안에서 제한된 페이지까지만 확인
  - 일부 페이지만 확인한 경우 그 사실을 warnings에 노출
- 적용 툴:
  - `value_up_v2`
  - `shareholder_meeting`
  - `dividend_v2`
  - `proxy_contest`
- 결과:
  - 현대자동차 `2024-01-01 ~ 2026-04-19` 조회 시 DART `20240828800218` 정상 복구
  - 기본 최근 12개월 조회에서는 `exists_outside_requested_window`로 과거 존재 여부를 구분
  - `공시가 없음` / `요청 구간 밖에 있음` / `형식상 수치 분석 불가`를 더 명확히 구분 가능
- 문서:
  - `dart-kind-disclosure-taxonomy.md`를 wiki source 문서로 반영
  - DART A~J / KIND 공시군 기준으로 v2 source policy를 재정리
