...
def register_tools(mcp):
    ...
    async def evidence(
        ...
        """desc: 인용 정보 제공자. rcept_no → 공시일·소스·뷰어 URL 유도. **API 호출 없음, 원문 추출 없음** — 순수 메타 가공기.
        when: evidence_refs 출처 재확인. raw rcept_no의 공시일/소스 빠른 판단.
        rule: 앞 8자리 → rcept_dt. 9~10자리 → source_type(80=수시공시, 그 외=DART). viewer_url은 DART 뷰어로 통일. report_nm은 upstream evidence_refs에만 있음. 14자리 숫자 아니면 requires_review.
