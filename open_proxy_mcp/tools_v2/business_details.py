...
def _render(p: dict) -> str:
    ...
    _FIELD_LABEL = {"sites": "사업장·생산설비", "utilization": "생산실적·가동률",
                    ...
                    "real_estate": "실물자산 장부가 vs 공정가치(자산저평가주)",
...
def register_tools(mcp):
    ...
    async def business_details(
        ...
        fields: 쉼표구분 — 표준: `segments,sites,utilization,rnd,backlog,customers` / 금융·REIT: `financial_ops,financial_soundness,investment_property` / **자산가치(opt-in, 명시해야 실행)**: `real_estate`(토지·투자부동산 장부가 vs 공정가치·재평가 — **자산저평가주** 스크리닝, III.재무 주석) · `equity_holdings`(지분증권 원가 vs 시가). (미지정 시 회사에 맞는 표준·금융 필드만, 자산가치 필드는 제외).
