...
def raw_cell(text: Any, inline: bool = False) -> str:
    ...
    자리가 정해진 칸(날짜·DPS·금액)은 파싱해서 열로 내도 된다. 그러나 회사가 무엇이든
    적는 칸은 정규식으로 한 가지를 뽑는 순간 나머지가 사라진다 — 읽는 쪽이 LLM 이므로
    원문을 그대로 넘기고 판단은 읽는 쪽에서 한다(260903 마스터 지시).

    하는 일은 **구조 보호뿐**이다. 내용은 한 글자도 버리지 않는다.
      · 줄바꿈 → 공백 (md 블록이 중간에 끊기지 않게)
      · `inline=True` 일 때만 `|` → `\\|` (표 셀에 넣을 때 열이 밀리지 않게)
    실측(결정공시 비고 3,831건): 줄바꿈 0건 · 파이프 0건 — 평소엔 아무것도 바뀌지 않고,
    서식이 달라진 원문이 들어와도 표가 깨지지 않는다.
    """
    s = str(text or "").replace("\r\n", " ").replace("\r", " ").replace("\n", " ").strip()
    if not s:
        return ""
    return s.replace("|", "\\|") if inline else s
