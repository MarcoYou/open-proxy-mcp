...
def register_tools(mcp):
    ...
    async def governance_report(
        ...
        """desc: 거버넌스/지배구조(corporate governance) 종합 보고서 — 주주총회(AGM) + 지분(OWN) + 배당(DIV) 3개 도메인 통합.
        when: [tier-4 Orchestrate] 거버넌스, 지배구조, corporate governance 전체를 한 번에 파악할 때. 주총 안건/투표결과 + 지분구조 + 배당 이력을 종합적으로 볼 때.
