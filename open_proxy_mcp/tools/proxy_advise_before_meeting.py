...
def _render(payload: dict[str, Any]) -> str:
    ...
    if guideline:
        ...
        if guideline.get("mode") == "pilot":
            ...
            lines.append(f"- 보팅 설정: {settings.get('stance')} · {settings.get('automation')} · 판단 기조 {settings.get('decision_posture', 0.75)} (0~1) · 실제 투표 제출 없음")
...
def register_tools(mcp):
    ...
    async def proxy_advise_before_meeting(
        ...
        guideline_workflow: pilot 사용자 설정. decision_posture=0~1(기본0.75, 높을수록 근거 있는 찬반 판단 선호; 핵심 충돌·사후정보·고정 기준 우회 불가), stance=standard|conservative, automation=automatic|selective|manual, manual_agenda_titles=수동 처리할 정확한 안건명 목록, attendance_min_pct=50~100 선택. 설정 변경 시 새 과업으로 평가. 자동은 권고안 준비이며 실제 투표 제출 기능은 없음. 누락·미기재는 원문 검토 후 unresolved_kind=missing_information으로 명시해 해당 기준만 제외하고 계속 판단. 자료 충돌과 평가 미실행은 누락과 구분.
