...
def _render(payload: dict[str, Any]) -> str:
    ...
    if guideline:
        ...
        if guideline.get("mode") == "pilot":
            ...
            lines.append(f"- 보팅 설정: 개입 성향 {settings.get('stance')} · 자동화 {settings.get('automation')} · 판단 기조 {settings.get('firmness', 0.75)} (0~1) · 실제 투표 제출 없음")
    ...
    if decisions:
        ...
        for i, ag in enumerate(decisions, 1):
            ...
            automation_trace = ag.get('automation_trace') or {}
            if automation_trace.get('fallback_applied'):
                original = automation_trace.get('assessment_recommendation')
                lines.append(f"- 최종 방향의 출처: 사용자 자동화 기본 정책 · 원래 평가는 {_DECISION_KO.get(original, original) or '미결'} · 사실 판단과 구분")
            ...
            if risks:
                ...
                lines.append("- 위험 신호: 기본 정책으로 찬반 방향만 정했으며 위험 해소를 확인한 것이 아닙니다"
                             if automation_trace.get('fallback_applied') else "- 위험 신호: 확인된 항목 없음"
            ...
            if workflow:
                labels = {"ready_for_auto": "자동 처리 준비", "manual_review": "수동 검토",
                          "awaiting_assessment": "LLM 평가 대기", "not_applicable": "표결 비대상",
                          "assessment_error": "평가 입력 오류", "execution_pending": "찬반 결정·실행 조건 미완료"}
    ...
    #    기준이 붙으면 두 값이 **어긋난 것이 아니라 기준이 다른 값**임이 드러난다.
...
def register_tools(mcp):
    ...
    async def proxy_advise_before_meeting(
        ...
        guideline_workflow: pilot 사용자 설정. firmness=0~1(기본0.75, 높을수록 근거 있는 찬반 판단 선호; 핵심 충돌·사후정보·고정 기준 우회 불가), stance=0~1(기본0.5, 낮으면 임박한 금전 손실 중심·높으면 주주권과 책임 적극 검토), automation=0~1(기본0.5, 0=모두 사람 검토, 0 초과~0.5=찬성만 자동 준비, 0.5 초과~1 미만=찬반 자동 준비·미결 수동, 1=미결도 stance<0.5 찬성·그 외 반대 기본 정책으로 최종 방향 결정), manual_agenda_titles/manual_agenda_ids=수동 처리할 정확한 안건명/ID 목록(automation=1과 동시 사용 불가), attendance_min_pct=50~100 선택. 기본 방향은 automation_trace에 원래 평가와 분리해 표시. 미제출·거절 평가는 완료가 아니며 기존 표 배분·조건은 execution_pending으로 보존. 설정 변경 시 새 과업으로 평가. 자동은 권고안 준비이며 실제 투표 제출 기능은 없음. 누락·미기재는 원문 검토 후 unresolved_kind=missing_information으로 명시해 해당 기준만 제외하고 계속 판단. 자료 충돌과 평가 미실행은 누락과 구분.
        ...
        guideline_structure: 같은 harness에서 {}로 선출 구조 과업을 요청. {protocol:staged}는 사실→정책 해석→권고→QA를 나누고 인용된 ballot_scope·현재 근거에 결합한 reviews를 요구한다. 응답 guideline_application.structure_tasks의 task 및 item_schemas를 읽고 같은 protocol의 {assessments:[평가]}로 제출. 정원·임기·시차·분리/집중선출·선행 조건을 원문 인용으로 평가. 자동 정책 추천과 수동 검토 분리, 사람 미검토. 정관 첨부 목록은 evidence_sources의 {type:dart_attachments,rcept_no}, 읽기는 {type:dart_attachment,rcept_no,dcm_no}. 이미지 원문은 등록된 visual reader로 판독하며 없으면 페이지와 needs_visual_reading을 반환. 미독을 미공개로 처리하지 않는다.
        guideline_research: guideline_harness와 함께 쓰는 목록 탐색. kind=meeting_resolution|periodic_reports|officer_changes|ownership_disputes|charter_history|legal_precedents, 선택 start_date/end_date(YYYYMMDD), page(1~20), page_count(1~100). 같은 회사·마감시점 안의 공시 목록과 다음 검색을 research_discovery로 반환한다. charter_history는 정기보고서 기준점과 마감 전 정기·임시주총의 변경 제안·결과·정정을 함께 탐색하고 첨부 읽기 요청을 준다. legal_precedents는 search_text(EUC-KR 40바이트 이하)로 공식 법원 판례속보 한 페이지를 검색한다(start_date·page_count 변경 미지원). 판례 읽기는 evidence_sources의 {type:court_precedent,board:precedent_bulletin,seqnum}. 공식 게시일을 확인한 요약만 반환하며 첨부 전문은 미독 상태다. 제목은 단서이며 아직 읽지 않은 원문이다. 모델이 관련 자료를 골라 guideline_evidence_sources로 읽은 뒤에만 평가 근거로 사용한다. 목록 0건·탐색 실패는 관계나 사건의 부재가 아니다.
