...
def register_tools(mcp):
    ...
    async def risk_events(
        ...
        """desc: 기업 리스크 이벤트 공시 통합 — 중대재해(산재·사망사고) / 횡령·배임 / 생산중단·영업정지 / **매매거래정지 / 관리종목·상장적격성 / 소송·제재 / 자본잠식**. 본사·종속/자회사 변형 포함. company 미지정(공백)이면 **시장 전체 최근 30일(최대 3개월) 스캔** — "최근 사고·사건 터진 기업들". include_details=True면 카테고리별 값 + **공시 원문 전문**을 같이 싣는다.
        ...
        rule: DART list.json I001(거래소 주요경영사항)+B001(주요사항보고서)+I003(거래소 시장조치·안내) 채널 + 키워드 — 중대재해는 305사 3.5년 차집합 0 검증, 본문 파싱은 연속 2개 90일 윈도우 359건 전수 audit. 관리종목·거래정지 계열은 I003 에만 있다(2026-08-27 전수 실측). company 지정 시 24개월 / 미지정 시 30일(최대 3개월 — DART 하드 제약, 일수가 아니라 역월). 주의: 중대재해 수시공시 신설 2025-10 — 이전 무공시 ≠ 무사고. 공시는 대형 원청·지주사 집중 — 비상장 자회사 사고는 상장 모회사가 공시.
