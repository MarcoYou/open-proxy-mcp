...
DART API는 **분당 1,000회 한도(IP 공유 — 초과 시 24시간 차단)**. 여러 회사를 짧은 시간에 몰아
조회하면 과호출로 일부 요청이 실패한다(상태코드 `020` = 요청 한도 초과). 이건 클라이언트를 바꾼다고
해결되지 않는다(같은 서버·같은 IP 한도).

- **여러 회사 조회 시**: 한 번에 1~2곳씩, 사이에 잠깐 간격을 두고 나눠서 호출한다(한꺼번에 수십 사
  병렬 금지). 오케스트레이터로 끝낼 수 있으면 detail tool 반복 호출을 피한다.
- **수십~수백 사 규모의 포트폴리오·시장 스캔**은 claude.ai 웹 세션(대화 길이·툴콜 상한)보다
  **Claude Code**(긴 세션·많은 툴콜)나 **open-proxy-ai**(서버측 배치·스로틀 내장)가 적합하다.
  대량 작업은 처음부터 이 환경에서 설계하는 편이 실패·재시도를 줄인다.
- 실패가 나면 응답의 안내를 따른다 — `020/021`(과호출)이면 건수를 줄이고, `timeout`이면 잠시 후
  재시도(사용자 방식 문제 아님), `no_data`면 재시도 대신 조건을 바꾼다.
