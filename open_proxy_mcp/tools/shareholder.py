...
def register_tools(mcp):
    ...
    async def agm_search(
        ...
        """주주총회 소집공고를 검색합니다. 다른 tool을 쓰기 전에 먼저 이 tool로 rcept_no를 확보하세요.
    ...
    async def agm_agenda_xml(
        ...
        정상: 안건 1개+, 제목 2-150자. 실패: 빈 결과, 제목 200자+ -> agm_agenda_pdf.
    ...
    async def agm_financials_xml(
        ...
        정상: BS 5행+, IS 3행+. 실패: BS/IS 없음 -> agm_financials_pdf.
    ...
    async def agm_personnel_xml(
        ...
        정상: 이름 2-4자, 경력 1건+, 각 100자 이내. 불완전: 경력 100자+ 병합. 실패: 빈 결과 -> agm_personnel_pdf.
    ...
    async def agm_aoi_change_xml(
        ...
        정상: 변경전/변경후 5자+. 실패: 빈 결과 -> agm_aoi_change_pdf.
    ...
    async def agm_compensation_xml(
        ...
        정상: limitAmount > 0. 불완전: limitAmount 없음. 실패: 빈 결과 -> agm_compensation_pdf.
    ...
    async def agm_treasury_share_xml(
        ...
        정상: items 1개+. 실패: 빈 결과 -> agm_treasury_share_pdf.
    ...
    async def agm_capital_reserve_xml(
        ...
        정상: amount 추출됨. reducedCapital=true면 감액배당 가능 기업. 실패: 빈 결과 -> agm_capital_reserve_pdf.
    ...
    async def agm_retirement_pay_xml(
        ...
        정상: 현행/개정안 비교 1건+. 실패: 빈 결과 -> agm_retirement_pay_pdf.
...
"""
    ...
    async def agm_personnel_pdf(
        ...
        agm_personnel_xml 실패 시 사용. 여전히 실패하면 agm_personnel_ocr을 시도하세요.
        ...
        if not result.get("appointments"):
            return "PDF에서도 선임/해임 안건을 찾을 수 없습니다. agm_personnel_ocr을 시도하세요."
    ...
    async def agm_personnel_ocr(
        ...
        agm_personnel_pdf도 실패했을 때 최후 수단으로 사용하세요.
    ...
    async def agm_financials_pdf(
        ...
        agm_financials_xml 실패 시 사용. 여전히 실패하면 agm_financials_ocr을 시도하세요.
        ...
        if not has_data:
            return "PDF에서도 재무제표를 찾을 수 없습니다. agm_financials_ocr을 시도하세요."
    ...
    async def agm_financials_ocr(
        ...
        agm_financials_pdf도 실패했을 때 최후 수단으로 사용하세요.
    ...
    async def agm_aoi_change_pdf(
        ...
        agm_aoi_change_xml 실패 시 사용. 여전히 실패하면 agm_aoi_change_ocr을 시도하세요.
        ...
        if not result.get("amendments"):
            return "PDF에서도 정관변경 사항을 찾을 수 없습니다. agm_aoi_change_ocr을 시도하세요."
    ...
    async def agm_aoi_change_ocr(
        ...
        agm_aoi_change_pdf도 실패했을 때 최후 수단으로 사용하세요.
    ...
    async def agm_compensation_pdf(
        ...
        agm_compensation_xml 실패 시 사용. 여전히 실패하면 agm_compensation_ocr을 시도하세요.
        ...
        if not result.get("items"):
            return "PDF에서도 보수한도 안건을 찾을 수 없습니다. agm_compensation_ocr을 시도하세요."
    ...
    async def agm_compensation_ocr(
        ...
        agm_compensation_pdf도 실패했을 때 최후 수단으로 사용하세요.
    ...
    async def agm_treasury_share_pdf(
        ...
        agm_treasury_share_xml 실패 시 사용. 여전히 실패하면 agm_treasury_share_ocr을 시도하세요.
        ...
        if not result.get("items"):
            return "PDF에서도 자기주식 안건을 찾을 수 없습니다. agm_treasury_share_ocr을 시도하세요."
    ...
    async def agm_treasury_share_ocr(
        ...
        agm_treasury_share_pdf도 실패했을 때 최후 수단으로 사용하세요.
    ...
    async def agm_capital_reserve_pdf(
        ...
        agm_capital_reserve_xml 실패 시 사용. 여전히 실패하면 agm_capital_reserve_ocr을 시도하세요.
        ...
        if not result.get("items"):
            return "PDF에서도 자본준비금 안건을 찾을 수 없습니다. agm_capital_reserve_ocr을 시도하세요."
    ...
    async def agm_capital_reserve_ocr(
        ...
        agm_capital_reserve_pdf도 실패했을 때 최후 수단으로 사용하세요.
    ...
    async def agm_retirement_pay_pdf(
        ...
        agm_retirement_pay_xml 실패 시 사용. 여전히 실패하면 agm_retirement_pay_ocr을 시도하세요.
        ...
        if not result.get("amendments"):
            return "PDF에서도 퇴직금 규정을 찾을 수 없습니다. agm_retirement_pay_ocr을 시도하세요."
    ...
    async def agm_retirement_pay_ocr(
        ...
        agm_retirement_pay_pdf도 실패했을 때 최후 수단으로 사용하세요.
    ...
    async def agm_agenda_pdf(
        ...
        agm_agenda_xml 실패 시 사용. 여전히 실패하면 agm_agenda_ocr을 시도하세요.
        ...
        if not result:
            return "PDF에서도 안건을 찾을 수 없습니다. agm_agenda_ocr을 시도하세요."
    ...
    async def agm_agenda_ocr(
        ...
        agm_agenda_pdf도 실패했을 때 최후 수단으로 사용하세요.
