...
def register_tools(mcp):
    ...
    async def screener(
        ...
        period: **사람 말로 받는다** — "오늘"/"어제"/"어제부터"/"지난주"(달력의 지난주 월~일)/"지난달"(지난달 1일~말일)/"최근 7일"/"지난 한 달"(굴러가는 30일)/"최근 3개월"/"최근 45일", 날짜범위 "20260801~20260820"·"2026-08-01~2026-08-20", 단일일 "20260820". 또는 `start_date`/`end_date`(레포 공통 인자, YYYYMMDD). 옛 코드(today/yesterday/since_yesterday/last_7d/last_30d/custom+custom_start·custom_end)도 그대로 동작. 디폴트 since_yesterday. 시장스캔 3개월 하드캡.
        ...
        view: 비우면 카드 보기(위 설명 전부). **"흐름"이면 흐름 보기** — 매일 밤 쌓이는 공시 원장만 읽어(DART 0콜) ① WICS 대분류·중분류별 새 공시 건수·금액을 직전 N주 같은 일수 환산 평소와 비교(「평소 대비 몇 배」) ② 매출 대비 비율이 기준 이상인 큰 수주 목록 ③ 올해 회사별 누적 수주(공시에 적힌 매출 대비 %의 합)를 준다. "지난주 업종별 수주 평소보다 많이 떴나"·"올해 수주가 매출 대비 가장 많이 쌓인 회사"·"업종별 자사주·증자 흐름"은 이것. 흐름 보기의 types 기본은 수주, period 기본은 원장 최신일까지 최근 7일(「올해」·「이번주」·「이번 달」·「지난달」·「최근 30일」·날짜범위도 받는다, 3개월 한도 없음 — 「지난주」「지난달」은 두 보기 모두 달력의 주·달, 흐름 보기의 「이번 달」은 1일~원장 최신일), universe 는 카드 보기와 같은 문법. 정정·해지는 새 공시로 세지 않고 따로 센다. 원장은 밤 배치라 오늘 뜬 공시는 카드 보기로.
