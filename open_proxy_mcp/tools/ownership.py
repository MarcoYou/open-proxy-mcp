...
def _format_major_shareholders(data: dict, changes: dict | None = None) -> str:
    ...
    for item in items:
        ...
        lines.append(f"| {name} | {relate} | {stock_knd} | {end_co} | {end_rt}% |")
    ...
    if changes and changes.get("list"):
        ...
        for ch in changes["list"]:
            lines.append(
                ...
                f"| {ch.get('qota_rt', '')}% | {ch.get('change_cause', '')} |"
            )
...
def register_tools(mcp):
    ...
    async def own(
        ...
        """지분 구조 종합 오케스트레이터.
        종목코드만으로 최대주주, 주식총수, 자사주, 5% 대량보유를 한 번에 반환.
        상세 분석은 개별 own_* tool 사용. API 6회+ 사용.
        ...
        try:
            treasury = await client.get_treasury_stock(corp_code, bsns_year)
        except DartClientError:
            treasury = empty
        ...
        # 마크다운 종합
        sections = [f"# {corp_name} 지분 구조 ({bsns_year} 사업보고서 기준)\n"]
        ...
        if major_items:
            ...
            sections.append(
                f"**최대주주**: {top.get('nm', '')} "
                f"({top.get('trmend_posesn_stock_qota_rt', '')}%, "
                f"{top.get('stock_knd', '')})\n"
            )
        ...
        for item in stock_items:
            if "보통" in item.get("se", ""):
                sections.append(
                    f"**보통주**: 발행 {_format_number(item.get('istc_totqy', ''))}주, "
                    f"자기주식 {_format_number(item.get('tesstk_co', ''))}주, "
                    f"유통 {_format_number(item.get('distb_stock_co', ''))}주\n"
                )
                break
        ...
        if block_items:
            ...
            for item in block_items:
                name = item.get("repror", "")
                dt = item.get("rcept_dt", "")
                if name and (name not in latest or dt > latest[name].get("rcept_dt", "")):
                    latest[name] = item
            sections.append("**5% 대량보유**:")
            for name, item in sorted(latest.items(), key=lambda x: float(x[1].get("stkrt", 0) or 0), reverse=True):
                purpose = _parse_holding_purpose(item.get("report_tp", ""), item.get("report_resn", ""))
                sections.append(f"- {name}: {item.get('stkrt', '')}% ({purpose})")
            sections.append("")

        sections.append(f"*상세: own_major, own_total, own_treasury, own_block, own_treasury_tx, own_latest 사용*")
        sections.append(f"*API 호출: 6회*")
