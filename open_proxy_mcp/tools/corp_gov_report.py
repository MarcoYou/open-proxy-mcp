...
def _render(payload: dict[str, Any], scope: str) -> str:
    ...
    if scope == "flags":
        ...
        if not fl:
            # 제출이 없어서 빈 것과 원문을 못 읽어서 빈 것은 뜻이 전혀 다르다.
            lines.append(
                "세부 준수 플래그를 읽지 못했습니다 — 미준수라는 뜻이 아닙니다. 원문을 확인하세요."
                if data.get("rcept_no")
                else "제출된 기업지배구조보고서가 없어 세부 준수 플래그도 없습니다."
            )
        else:
            lines.extend([
                f"## 세부 준수 플래그 ({len(fl)}개 · Y {data.get('flags_yes')} / "
                f"N {data.get('flags_no')})",
                "> Y 개수는 준수율이 아니다. 준수가 아니라 **사실**을 묻는 항목이 섞여 있고"
                " (「주주제안 내역」 N = 주주제안이 없었다), 방향이 반대인 것도 있다"
                " (「불성실공시법인 지정」 **Y 가 제재를 받았다**는 뜻). 항목별로 읽어야 한다.",
                "",
                # 질문문 10종은 한 줄로 두 가지를 묻는다(「선임 사외이사, 집행임원제도 도입 여부」).
                # 그래서 답 항목을 따로 세워야 어느 쪽 답인지 갈린다.
                "| # | 원칙 | 서식이 묻는 것 | 답 항목 | 답 | 15지표 |",
                "|---|------|--------------|--------|----|--------|",
            ])
            for i, f in enumerate(fl, start=1):
                # ✅/❌ 를 달지 않는다 — 「제재 내역 N(없음)」에 ❌ 가 붙으면 좋은 답이 나쁘게 읽힌다.
                same = f.get("same_as_metric", "")
                lines.append(
                    f"| {i} | {f.get('principle', '')} | {f.get('question', '')[:100]} "
                    f"| {f.get('label', '')[:34]} | {f.get('value', '')} "
                    f"| {('= ' + same[:40]) if same else ''} |"
                )
...
def register_tools(mcp):
    ...
    async def corp_gov_report(
        ...
        scope: `summary` 기업개요+준수율+15지표 / `metrics` 15지표 + 비고 상세 / `principles` 세부원칙 응답 / `filings` 제출 이력 / `timeline` 연도별 추이 + 지표 전환 / `tables` 서식 표 원본 11종 — 주총 운영(1-1-1 소집공고~주총 실제 일수·개최장소·감사 출석 · 1-2-1 집중일 회피·서면/전자투표 · 1-2-2 안건별 찬반 주식수) · 이사(4-1-2 이사회 구성·전문분야·주요경력 · 4-2-1 선임·변동사유 · 4-3-1 후보 사전 정보제공기간 · 5-2-1 사외이사 겸직 · 7-1-1 이사회 개최·안건통지 간격 · 7-2-1 개별이사 3개년 출석률·찬성률) · 감사(9-1-1 내부감사기구 구성·재무전문가 · 10-2-1 외부감사인 소통내역) / `flags` 본문 세부 준수 플래그 78개 Y/N — 15지표를 잘게 쪼갠 질문이라 72개는 15지표에 없는 사실(주주제안·공개서한 접수 · 재무제표 6주전/연결 4주전 제공 · 영문자료 · 선임사외이사·집행임원 제도 · 사외이사 개별평가 · 감사기구 지원조직 설치·독립성 · 외부감사인 독립성 훼손우려 · 기업가치제고계획 자율공시 등)
        year: 사업연도 지정 (0이면 최신).
