...
def _is_valid_candidate_name(name: str) -> bool:
    ...
    if any(kw in name for kw in ['보수총액', '최고한도', '실제지급', '주식의종류', '주식의 종류']):
        return False
    # 직위·기구 이름은 사람이 아니다. 안건 제목 꼬리를 이름으로 읽으면 실재하지 않는 후보가 생기고,
    # 그 유령에 「독립적·결격사유 없음」 평가까지 붙는다 — 실측 코웨이 주주제안 「사외이사 이사회 의장
    # 선임의 건」이 후보 「이사회 의장」을, 덴티움 「감사위원회위원 전원 사외이사 구성」이 후보 「구성」을
    # 만들었다(코웨이는 실제 후보 8명인데 9명으로 보고). 아래 catch-all 분기가 이런 말을 통과시킨다.
    # 사람 이름에 섞일 수 없는 기구·직위어는 **부분 일치**로 막는다.
    if any(kw in name for kw in (
        "이사회", "위원회", "대표이사", "사외이사", "사내이사", "감사위원",
        "집행임원", "비상무이사", "후보자",
    )):
        return False
    # 「구성」·「선임」은 이름에도 쓰이는 글자다(박구성 등) — **이름 전체와 같을 때만** 막는다.
    # 부분 일치로 막으면 실재 후보가 조용히 사라진다.
    if name.replace(" ", "") in ("의장", "구성", "전원", "선임", "해임", "이사", "감사", "위원"):
        return False
