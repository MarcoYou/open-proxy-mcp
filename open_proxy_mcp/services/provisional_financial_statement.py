...
_METRIC_KEYWORDS = {
    ...
    # 보험사 top line 은 IFRS 1117호(2023-01-01 시행) 전후로 이름이 다르다 — 구 1104호는
    # 「보험영업수익」, 1117호는 「보험서비스수익」·「보험수익」이다. 캐시 실측: 보험영업수익 7 ·
    # 보험수익 4 · 보험서비스수익 2. 접두 매칭이라 **「보험서비스비용」(비용)·「출재보험서비스수익」
    # (출재 = 나가는 쪽)·「일반보험서비스수익」(부문 소계)은 자동으로 안 걸린다** — 앞이 다르다.
    "revenue_krw": ("매출액", "매출", "수익(매출액)", "영업수익", "수익 (매출액)",
                    "보험영업수익", "보험서비스수익", "보험수익", "영업수익(매출액)",
                    # 건설 — 1115호 진행기준. 실측 「공사매출」 3건.
                    "공사매출", "공사수익", "분양매출", "분양수익"),
...
_METRIC_EXCLUDE: dict[str, tuple[str, ...]] = {
    ...
    # 손익 항목도 막는다 — 「비지배지분순손익」·「비지배지분총포괄순손익」은 자본이 아니라
    # 그 해 손익의 귀속분이다(실측 20260310002815). 자본 계정으로 읽으면 자본총계에서 그만큼
    # 잘못 빼게 되고, 그 차이가 그대로 지배지분 오산으로 간다.
    "nci_krw": ("비지배지분부채", "비지배지분율", "비지배지분의장부금액", "비지배지분에",
                "비지배지분으로", "비지배지분의증감", "비지배주주지분(부채", "비지배주주지분(자본",
                "비지배지분순", "비지배지분총포괄", "비지배지분귀속", "비지배주주지분순",
                "비지배주주지분귀속"),
}
...
#: 배제도 **손실형을 같이 적어야 한다.** 「매출총이익」만 두면 적자 회사의 「매출총손실」이
#: 매출로 걸린다 — 영풍 손익계산서가 실제로 「Ⅲ.매출총손실」이다. 이익형만 적는 습관이
#: 순이익 쪽에서 3조 6,027억 사고를 냈고, 여기에도 같은 구멍이 남아 있었다.
_REVENUE_EXCLUDE = ("매출원가", "매출총이익", "매출총손실", "매출총손익",
                    "매출채권", "매출할인", "매출에누리", "매출환입",
...
def extract_metrics(parsed: dict[str, Any], prefer: str = "consolidated") -> dict[str, Any]:
    ...
    for scope in scope_order:
        ...
        for table, stmt_type in ((income, "income_statement"), (balance, "balance_sheet")):
            ...
            # **금융사는 총계도 유동/비유동 구분도 안 적는다** — K-IFRS 1001호 유동성 순서
            # 배열이라 「자 산」·「부 채」·「자 본」 섹션 헤더만 두고 그 아래로 계정을 나열한다
            # (실측 20260309001843: 48행 재무상태표에 「총계」 문자가 한 번도 안 나온다).
            # 총계류만 앵커로 두면 그런 표를 통째로 막아 자본금·지배지분이 있는데도 못 뽑는다.
            # 섹션 헤더 정확일치와 자본 섹션 고유 계정을 함께 본다 — 현금흐름표·자본변동표·
            # 종속기업 요약표에는 이것들이 없다.
            _anchors = ("자산총계", "부채총계", "자본총계", "유동자산", "비유동자산",
                        "유동부채", "비유동부채", "자산총액", "부채총액",
                        "부채및자본", "부채와자본", "자본잉여금", "이익잉여금", "결손금")
            _section = {"자산", "부채", "자본"}
            looks_like_bs = sum(
                1 for a in account_lines
                for _c in (_strip_item_marker((a or "").replace(" ", "")),)
                if _c.startswith(_anchors) or _c in _section
            ) >= 2
