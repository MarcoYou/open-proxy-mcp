...
설계: III.재무 주석은 서식 변형이 크고 **DART HTML이 중첩 <table>이라 grid 파싱이 헤더만 떠내고
데이터행을 놓친다**. → table 파싱 포기, **stripped 텍스트 region 윈도**를 마크다운으로 반환
(markdown-primary 순수형 — 텍스트에는 데이터가 다 있음, 호출측 AI가 읽음). 앵커 literal +
content-signature(순수 lookahead)로 region을 지목하고, 산문 회계정책·BS 한줄·CF조각은
signature로 배제. [[markdown-primary-anchor-260719]].
"""
...
# ── 연결/별도 기준 ────────────────────────────────────────────────────────
# DART document.xml 은 주석 표의 셀마다 XBRL 컨텍스트를 단다:
#   ACONTEXT="…ConsolidatedAndSeparateFinancialStatementsAxis_ifrs-full_ConsolidatedMember…"
# 기간과 연결/별도가 여기에 선언돼 있다. **선언이 없으면 기준을 내지 않는다** —
# 별도 자산을 연결로 오인하면 NAV 가 달라지므로 추정으로 메우지 않는다(correct-or-absent).
_ACONTEXT_RE = re.compile(r"""ACONTEXT\s*=\s*["']([^"']{1,400})""", re.I)
_UNIT_PREFIX_RE = re.compile(r"^\[단위:[^\]]*\]\s*")
# 주석 절 제목 — 셀 선언과 독립된 둘째 신호. 둘이 어긋나면 어느 쪽이 틀린 건지 알 수 없으니
# 조용히 고르지 않고 경고로 드러낸다.
...
def _strip_with_map(html: str) -> tuple[str, list[int]]:
    ...
    결과 region 만으로는 ACONTEXT 를 되찾을 수 없고, 문구를 html 에서 다시 찾는 방식은
    공백 축약 탓에 어긋난다. 그래서 변환을 글자 단위로 따라가며 대응표를 만든다.
    """
    out: list[str] = []
    pos: list[int] = []
    last = 0
...
# 「우리가 못 읽었다」고 말하려면 그 필드의 명세여야 한다. 어휘만 스친 다른 표
# (가격위험 민감도·공정가치 서열·재무상태표 줄·차입금 내역)는 미탐이 아니다.
_DETAIL_WINDOW = 600
_DETAIL_HINTS = {
...
def _body_pos(stripped: str, anchors: tuple[str, ...]) -> int | None:
    ...
    정기보고서 앞머리 목차에 「우발부채」·「지분증권」이 그대로 실려 있어, 첫 위치를 그냥
    쓰면 목차를 읽고 판정하게 된다. 목차 줄의 숫자는 쪽번호뿐이라 여섯 자리 금액이나 단위
    선언이 곁에 있는 위치만 본문으로 친다.
    """
    best = None
    for a in anchors:
...
def _excerpt_at(stripped: str, pos: int, before: int = 40, after: int = 70) -> str:
    ...
    주석 번호를 되짚어 뽑으면 한 칸 앞 주석이나 II장 소제목을 집는 일이 잦다 — 틀린 번호는
    읽는 쪽을 엉뚱한 데로 보내니 없느니만 못하다. 대신 그 자리의 원문을 그대로 인용한다.
    """
...
#   · 변동표는 토지 기말잔액에 '장부금액' 라벨이 없고 기초/취득/기말 컬럼만 쓴다.
#   · 롤포워드 컬럼이 없는 스냅샷 서식은 '기초'가 없어 '전기말'을 대안으로 둔다(리터럴 동의어).
# 담보제공/현금흐름표 오탐은 markdown-primary(caller가 읽어넘김) + 멀티소스 계정대사로 흡수한다.
_SIG_TANGIBLE_LAND = re.compile(
    r"(?=.*토지)(?=.*[\d,]{6,})(?=.*(?:취득원가|총장부금액|기초|전기말))(?=.*(?:장부금액|장부가액|기말))", re.S)
# 투자부동산 명세: 취득원가/취득가액·(감가)상각누계액·장부금액 + 토지/건물 + 5자리(산문·CF 배제).
# '원가 또는 간주원가'(IFRS1 최초채택 표준문구)·'취득금액'은 취득원가의 실무 동의어라 함께 받는다.
_SIG_INV_PROP = re.compile(
    r"(?=.*투자부동산)(?=.*(?:취득원가|취득가액|총장부금액|취득금액|간주원가))(?=.*장부금액)"
    r"(?=.*(?:토지|건물)\s*[\d,]{5,})", re.S)
# 재평가(FV 반영): '재평가적립금/잉여금' 키워드에 **값이 인접**(자본변동표 자본금 오긁음 방지).
...
# 금융자산 지분증권 원가 vs 시가. '지분율'만 요구하면 종속/관계기업투자(지분법, 시가 아님)와
# 회계정책 산문을 긁는다 → **상장주식/비상장주식**(FVOCI/FVPL 명세 고유) + 원가축 +
# (공정가치|순자산가액|평가손익) 을 함께 요구한다. 취득가액·총장부금액도 원가 개념의 실무 동의어다.
# 단 '총장부금액'은 K-IFRS9 매출채권 손상충당금 표에도 쓰이므로, 그 표 고유 용어인 '손상차손누계액'
# (지분증권 표는 '차손익적립금'/'평가손익'을 쓴다)이 공존하면 배제해 매출채권 오탐만 걸러낸다.
_SIG_EQUITY = re.compile(
    r"(?!.*손상차손누계액)"
    r"(?=.*(?:취득원가|취득가액|총장부금액))(?=.*(?:상장|비상장)\s*(?:주식|지분상품|지분증권))"
    r"(?=.*(?:공정가치|순자산가액|평가손익))", re.S)
# FVPL 상장주식 롤포워드형 보유명세 — 위 _SIG_EQUITY는 "원가 vs
...
# 위 배제(`손상차손누계액`)는 총장부금액이 매출채권 손상충당금 표에도 쓰이기 때문에 넣은
# 것인데, 창 안 다른 주석에 그 낱말이 있으면 진짜 지분증권 명세까지 죽는다. 그래서 배제를
# **총장부금액만 있을 때로 한정**한 완화판을 둔다 — 취득원가·취득가액은 매출채권 표가 쓰지
# 않는 어휘다. 앵커 순서가 바뀌어 기존 구간이 밀리지 않도록 위에서 못 찾았을 때만 쓴다.
...
# 담보물 **종류**가 아니라 담보 표의 **열**로 지목한다. `_SIG_PLEDGED` 의 자산종류 화이트리스트
# (토지·건물·예금·주식…)는 축이 좁다 — 실제 담보물은 보험·지분증권·출자금·외화예치금·재고자산처럼
# 제각각이다. 종류는 갈려도 「누가 잡았나(담보권자·담보제공처)·얼마로 잡았나(담보설정금액·
# 채권최고액)」 열은 공통이다.
_SIG_PLEDGED_COLS = re.compile(
    r"(?=.*담보)"
    r"(?=.*(?:담보권자|담보설정금액|담보제공처|채권최고액|설정권자|질권금액))"
    r"(?=.*(?:장부금액|장부가액|담보설정|담보제공))(?=.*[\d,]{6,})", re.S)
# 축을 넓히면 「담보제공 내역 - 해당사항 없음」 같은 부재 선언 문단까지 들어온다.
...
def extract_real_estate(biz_text: str, full_html: str, stripped: str | None = None) -> dict[str, Any]:
    ...
    stripped: 이미 _strip한 텍스트가 있으면 전달(재strip 회피)."""
...
def extract_equity_holdings(biz_text: str, full_html: str, stripped: str | None = None) -> dict[str, Any]:
    ...
    txt = stripped if stripped is not None else _strip(full_html)
    # require 미지정: "취득원가" 단일리터럴 프리필터는 취득가액/총장부금액 표기를 과잉 배제한다 —
    # 앵커(상장/비상장주식)가 이미 좁아 프리필터 없이도 성능 영향이 작다.
    specs = [
...
def extract_pledged_assets(full_html: str, stripped: str | None = None) -> dict[str, Any]:
    ...
    if not regions:
        # 표를 이끄는 다른 문구들. 앵커를 「담보로 제공된/되어」처럼 넓히면 「…담보로 제공되어
        # 있습니다(주석 5참조)」 같은 산문에서 출발해 다음 주석 표를 끌어온다. 그래서 넓히지
        # 않고, 표 제목으로 쓰이는 문구만 골라 위에서 못 찾았을 때만 본다.
