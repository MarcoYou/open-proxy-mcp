...
_FOPS_HEAD = [r"영업의?\s*현황", r"영업\s*개황", r"영업부문별\s*(?:재무정보|비중|현황)",
              r"영업의?\s*종류"]
# content-gate: 금융 규제지표만(범용 '영업수익/점유율/운용자산'은 비금융사 오발 → 제거)
_C_FOPS = re.compile(r"순이자손익|보험영업손익|영업순수익|운용보수|수탁고|수입보험료|영업부문별\s*재무|"
                     r"예대율|지급여력비율|위탁매매수수료|집합투자업")
_SIG_FOPS = re.compile(r"순이자손익|영업부문별\s*재무|은행\s*부문.{0,20}증권|보험영업손익|영업순수익")
# 금융 재무건전성(지급여력·RBC·BIS·자본적정성)
_FSND_HEAD = [r"재무\s*건전성", r"지급\s*여력", r"자본\s*적정성"]
# 금융 규제지표만(자기자본비율 등 범용어 제거 — 비금융사 오발 방지)
_C_FSND = re.compile(r"지급여력비율|K-ICS|BIS\s*비율|RBC|순자본비율|영업용순자본|고정이하여신|연체율|"
                     r"책임준비금|위험가중자산")
