...
#: 주주총회를 가리키는 칸. 그 기수에 주총이 없었으면 회사는 「미개최(전기)」처럼 적으므로
#: 부재 표기도 이 축의 값으로 본다 — 신규 상장사는 전기·전전기가 전부 그렇다.
_AGM_KEY_RE = re.compile(r"주총|주주총회|총회|정기|임시|\d+\s*기|\d{4}|미개최|미해당|해당\s*없음")
