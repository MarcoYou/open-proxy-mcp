...
| 이사회 출석률 §2.4 against ④ | 75% 미만 against (8 운용사 공통 「3/4 룰」) | **엔진 미반영** (260903). 데이터는 있다 — `director_board`(scope=attendance: 사업보고서 「이사회 활동내역」 개별 출석률, <75% low flag)·`corp_gov_report`(scope=tables 표 7-2-1: 3개년 개별 출석률) — `proxy_advise` 가 읽지 않는다. 260903 10사 표본(FY2025 사업보고서): 이사회 전원 파싱 4(SK하이닉스·KT·한국앤컴퍼니·이마트) / 일부만 기재 3(에코프로비엠 1/9·리노공업 1/7·태광산업 5/7 — 주로 사외이사만) / 표 미발견 2(대림제지·카카오뱅크) / 원문 조회 실패 1(KB금융). 75% 미만 실측: 한국앤컴퍼니 조현범 33%·안종선 67% / SK하이닉스 박성하 50% — **퇴임 직전 3개월 재직이라 분모가 2회**(분모 왜곡) / KT 김영섭 75% 정확히 경계(정책은 「미만」이라 미해당). **미반영으로 두는 이유**: ① 시점 — 소집공고 시점엔 당기 사업보고서가 아직 없어(공고 +7일 중앙값) 전기 값을 읽게 되고, 지배구조보고서는 주총 시점 기준 항상 전전기다 ② 재직 짧은 이사의 분모 왜곡 ③ 표본 30%는 표 자체가 없다. **다음 단계**: 판정 트리거가 아니라 `facts` 로 먼저 노출(연도·출처 병기) → as_of 게이트 건 30사 표본 뒤 판정 반영 여부 결정. 가장 정직한 출처는 소집공고 자체의 「사외이사 등의 활동내역」(회차별 출석·찬반, 당기, look-ahead 없음) — 파서 미구현 |
