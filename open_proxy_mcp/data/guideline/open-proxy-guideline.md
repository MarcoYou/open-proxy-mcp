...
- v1.1에서 추가했던 B외국계-only novel topics 정리:
  ...
  - 유지: bundled_slate_against (A행동주의 동조), overboarded_directors (한국 상법 §382의2 + 시행령 §34 + K레거시 정량)
...
| 이사회 출석률 §2.4 against ④ | 75% 미만 against (8 운용사 공통 「3/4 룰」) | **엔진 미반영** (260903). 데이터는 있다 — `director_board`(scope=attendance: 사업보고서 「이사회 활동내역」 개별 출석률, <75% low flag)·`corp_gov_report`(scope=tables 표 7-2-1: 3개년 개별 출석률) — `proxy_advise` 가 읽지 않는다. 260903 10사 표본(FY2025 사업보고서): 이사회 전원 파싱 4(예시기업 AL·예시기업 FV·예시기업 GT·예시기업 EY) / 일부만 기재 3(예시기업 EG 1/9·예시기업 BU 1/7·예시기업 GE 5/7 — 주로 사외이사만) / 표 미발견 2(예시기업 BD·예시기업 FR) / 원문 조회 실패 1(예시기업 S). 75% 미만 실측: 예시기업 GT 조현범 33%·안종선 67% / 예시기업 AL 박성하 50% — **퇴임 직전 3개월 재직이라 분모가 2회**(분모 왜곡) / 예시기업 FV 김영섭 75% 정확히 경계(정책은 「미만」이라 미해당). **미반영으로 두는 이유**: ① 시점 — 소집공고 시점엔 당기 사업보고서가 아직 없어(공고 +7일 중앙값) 전기 값을 읽게 되고, 지배구조보고서는 주총 시점 기준 항상 전전기다 ② 재직 짧은 이사의 분모 왜곡 ③ 표본 30%는 표 자체가 없다. **다음 단계**: 판정 트리거가 아니라 `facts` 로 먼저 노출(연도·출처 병기) → as_of 게이트 건 30사 표본 뒤 판정 반영 여부 결정. 가장 정직한 출처는 소집공고 자체의 「사외이사 등의 활동내역」(회차별 출석·찬반, 당기, look-ahead 없음) — 파서 미구현 |
| 사외이사 장기연임 5년+·6년 초과 재직 §2.4 against ①·② | 둘 다 against 목록 | **REVIEW** — 5년은 OPM 자체 소프트 조기경보(성문 규정 없음), 6년 초과는 상법 시행령 §34⑤7호 결격 **가능**. 재직기간이 하한 추정(과소계상)이고 동일회사/계열 합산(9년)을 못 갈라 결격을 확정하지 않는다(260710 법률 정정). 감사위원도 같은 문턱 |
| 과도 겸임 §2.4 against ③ | 사외이사 3개사 / 상장사 임원 2개사 against | **REVIEW** — 타사 사외이사 **3곳 이상**(`concurrent_outside_directors=strong_concerns_concurrent`)만. 「상장사 임원 2개사」 축은 미구현 |

**인용 라벨과 이 표의 관계 (260903)**: `proxy_advise_before_meeting` 의 「정책 인용」 줄은
`OPM Guideline §2.4 이사 선임 — against ①「사외이사 장기연임 5년+」… ▸ 엔진: …` 처럼 **이 문서의 절 번호와
항목 번호**를 가리키고, `tests/test_policy_citations_match_document.py` 가 문서 헤딩·항목과 자동
대조한다. 이 문서의 항목을 고치거나 순서를 바꾸면 그 테스트가 먼저 깨진다 — 라벨을 같이 고칠 것.
라벨의 「▸ 엔진:」 뒤가 이 표의 세 번째 열과 같은 내용이다.
...
5개 한국 운용사(M레거시·S레거시·SA레거시·T행동주의·K레거시) 정책 합의 매트릭스만으로는 부족하다. 이유:
...
- **against**:
  ...
  - 이사회 규모 7명 미만 축소
- **review**: 회사명 변경 (판단 기준 — 모회사 종속 / 사업 정체성 / 무형가치)
...
- **against**:
  ...
  - 스톡옵션 2% 초과 또는 누적 희석 5%+ (T행동주의·K레거시 + A3 강화)
  - 사외이사 퇴직혜택, 황금낙하산
...
- **against**:
  ...
  - **분쟁 중 자사주 처분** (예시기업 HC KCGI 패턴) — A3 활동가
...
- **against**:
  - 주주가치 훼손 (5/5 합의)
  - 지배주주 이해상충 합병 + MoM 미적용 (예시기업 CQ-예시기업 FI 패턴)
  - 왕관보석/그린메일
...
- **against**:
  - **물적분할 + 자회사 상장** (2026.07 신법 + 예시기업 W 사례) — A1+A3+A5+A6+A7 5명 합의 (가장 강력한 신규 입장)
  - 분할 후 지배주주 비대칭 증가 (사익편취)
  - 분할 가치 산정 독립성 부재
  - **지주회사 설립** — 총수 지배력 강화 목적 (예시지주 D·예시지주 A·예시지주 B·예시지주 C 사례) — A3 활동가
- **review**:
  - 지주회사 설립 (총수 강화 목적 외)
...
- **against**:
  - 우선인수권 50%/20% 초과 (T행동주의·K레거시 + A3 강화 30%/10%)
...
- **for**:
  ...
  - 경영권 분쟁 시 신주발행/자사주 매각 주총 승인 요구 (예시기업 HC KCGI 패턴)
...
- **한국 상태**: 자율 도입 ~8% (KOSPI 5천억+). 예시기업 GK·예시기업 CP·S레거시SDS 도입
...
- **사례**: 예시기업 Y-예시기업 W (모회사 시총 약 30조 증발)
...
- **근거**: A5 — B외국계 2024, Climate Action 100+
...
- [x] v1.0 결정 (7 전문가 토론) 모두 보존 — 강화/보완만, 뒤집기 없음
